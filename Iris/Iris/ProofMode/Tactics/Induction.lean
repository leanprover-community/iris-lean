...
theorem revert_IH [BI PROP] {Q R P : PROP} {φ}
    (ih : φ)
    (h1 : P ⊢ □ P)
    (h2 : P ⊢ Q)
    (inst : IntoIH φ P R) :
...
private def addIH {u} {prop : Q(Type u)} {bi : Q(BI $prop)} {e}
  ...
  return { newHyps, pf := q(revert_IH $p $pfIntHyps $st.pf $inst) }
    ...
    fun hyps' goal' k => do
      ...
      for ⟨s, ctor⟩ in subgoals.toList.zip caseNames do
        s.mvarId.withContext do
          ...
          -- Label the induction subgoal for the user with a name
          let biGoalMVar := (← getThe ProofModeM.State).goals.back!
          biGoalMVar.setTag ctor
