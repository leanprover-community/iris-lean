...
theorem stay_intuitionistic [BI PROP] {P Q : PROP}
    (h1 : P ⊢ □ P) :
    P ∗ □ Q ⊢ □ (P ∗ □ Q) := calc
  _ ⊢ □ P ∗ □ Q   := sep_mono_l h1
...
private def addIntoIH {u} {prop : Q(Type u)} {bi : Q(BI $prop)} {e goal}
  ...
  let a := q(revert_IH $p $inst (goal := $goal) $st.pfAux)

  return {
    newHyps,
    pf := q(fun pf => $st.pf <| $a pf),
    pfAux := q(stay_intuitionistic $st.pfAux)
  }

/--
  This should be true in `addAllIHs` because all spatial hypotheses have
  been reverted, so the Iris proof state contains no hypothesis in the
  spatial context. -/
theorem dummy [BI PROP] {e : PROP} : e ⊢ □ e := sorry
...
private def addAllIHs {u} {prop : Q(Type u)} {bi : Q(BI $prop)} {e goal : Q($prop)}
  ...
  let mut st : InductionState e goal := {
    newHyps := hyps, pf := q(id), pfAux := q(dummy)
  }
