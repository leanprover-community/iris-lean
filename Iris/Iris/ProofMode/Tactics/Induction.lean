...
private def exprContainsConst (e : Expr) (n : Name) : Bool :=
  (e.find? fun
    | .const c _ => c == n
    | _ => false).isSome

/--
  Search for all induction hypotheses generated by Lean's built-in `induction`
  tactic and return their `FVarId` values as a list.
-/
private def findIHs : ProofModeM (List FVarId) := do
  let lctx ← getLCtx
  return ← lctx.decls.toList.filterMapM fun
    | none => return none
    | some ldecl =>
      if ldecl.isAuxDecl || ldecl.isImplementationDetail
        then return none
      else
        if exprContainsConst ldecl.type ``Entails'
        then return some ldecl.fvarId
        else return none
