...
theorem revert_IH [BI PROP] {P Q goal : PROP} {φ}
    (ih : φ)
    (inst : IntoIH φ P Q)
    (h1 : P ⊢ □ P)
    (h2 : P ∗ □ Q ⊢ goal) : P ⊢ goal := calc
  P ⊢ □ P         := h1
...
private def addIntoIH {u} {prop : Q(Type u)} {bi : Q(BI $prop)} {e goal}
  ...
  let a := q(revert_IH $p $inst (goal := $goal) $st.pfIntHyps)
...
private def addAllIHs {u} {prop : Q(Type u)} {bi : Q(BI $prop)} {e goal : Q($prop)}
  ...
  let mut st : InductionState e goal := {
    newHyps := hyps, pf := q(id), pfIntHyps
  }
      ...
      fun hyps' goal' k => do
        ...
        for s in subgoals do
          s.mvarId.withContext do
            ...
            let st ← addAllIHs pfIntHyps irisGoal.hyps ihFVars (goal := irisGoal.goal)
