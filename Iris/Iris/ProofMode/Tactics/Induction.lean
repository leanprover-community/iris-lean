      ...
      for ⟨s, ctor⟩ in subgoals.toList.zip recCtors do
        s.mvarId.withContext do
          s.mvarId.setTag ctor
            ...
            let ihFVars ← s.mvarId.withContext do
              return (← (← getLCtx).decls.toList.reduceOption.filterM
                (fun decl => do return containsIrisGoal (← instantiateMVars decl.type).getForallBody)).map (·.fvarId)
