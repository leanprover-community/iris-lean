      ...
      for ⟨s, ctor⟩ in subgoals.toList.zip recCtors do
        s.mvarId.withContext do
          s.mvarId.setTag ctor
            ...
            let ihFVars ← s.mvarId.withContext <| getLCtx >>= fun lctx =>
              (·.map (·.fvarId)) <$> lctx.decls.toList.reduceOption.filterM
                ((containsIrisGoal ∘ (·.getForallBody)) <$> instantiateMVars ·.type)
