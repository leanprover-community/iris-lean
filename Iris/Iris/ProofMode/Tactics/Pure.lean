...
def iPureCasesEntails {u} {prop : Q(Type u)} (bi : Q(BI $prop)) (ty : Q(Prop))
    (pat : TSyntax `rcasesPat)
  ...
  for g in gs do
    g.withContext do
      let some #[_, _, tm, goal'] := (← instantiateMVars (← g.getType)).consumeMData.appM? ``BIBase.Entails
...
def iPureCore {prop : Q(Type u)} {bi : Q(BI $prop)}
  ...
  let f ← iPureCasesEntails bi q($φ → ($(hyps'.tm) ⊢ $Q)) purePat k
  have : $hyps'.tm =Q $P' := ⟨⟩
...
elab "ipure " colGt hyp:ident : tactic => do
  ...
  let pf ← iPureCore e hyps' p out' goal (← `(rcasesPat| $hyp:ident)) pf (addBIGoal (bi := bi))
...
elab "ipure " colGt hyp:ident " with " pat:rcasesPat : tactic => do
  ...
  let pf ← iPureCore e hyps' p out' goal pat pf (addBIGoal (bi := bi))
