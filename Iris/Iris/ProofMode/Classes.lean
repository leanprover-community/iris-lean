...
class AsEmpValid0 (d : AsEmpValid.Direction) (φ : Prop) (io : InOut := d.toInOut)
    (PROP : semiOutParam $ inOutParam io <| Type _)
    (bi : semiOutParam $ inOutParam d.toInOut <| BI PROP) (P : outParam PROP) where
...
class FromWand {PROP} [BI PROP] (P : PROP) (io : InOut)
    (Q1 : semiOutParam <| inOutParam io PROP) (Q2 : outParam $ PROP) where
  from_wand : (Q1 -∗ Q2) ⊢ P
...
class IntoWand {PROP} [BI PROP] (p q : Bool) (R : PROP)
  (ioP : InOut) (P : semiOutParam <| inOutParam ioP PROP)
  (ioQ : InOut) (Q : semiOutParam <| inOutParam ioQ PROP) where
  into_wand : □?p R ⊢ □?q P -∗ Q
...
class FromAssumption {PROP} [BI PROP] (p : Bool) (ioP : InOut)
    (P : semiOutParam <| inOutParam ioP PROP) (Q : PROP) where
...
class FromPure {PROP} [BI PROP] (a : outParam $ Bool) (P : PROP) (ioφ : InOut)
    (φ : semiOutParam <| inOutParam ioφ Prop) where
