...
instance (priority := default + 10) asEmpValidEmpValid
    [bi : BI PROP] d (P : PROP) : AsEmpValid d (⊢ P) PROP bi P where
  as_emp_valid := by simp

@[rocq_alias as_emp_valid_entails]
instance asEmpValid_entails [bi : BI PROP] d (P Q : PROP) :
    AsEmpValid d (P ⊢ Q) PROP bi iprop(P -∗ Q) where
...
instance asEmpValid_forall {α} [bi : BI PROP] (Φ : α → Prop) (P : α → PROP)
    [hP : ∀ x, AsEmpValid d (Φ x) PROP bi iprop(P x)] :
    AsEmpValid d (∀ x, Φ x) PROP bi iprop(∀ x, P x) where
