...
instance fromModal_embed modStatus (P : PROP1) :
    FromModal (PROP1 := PROP1) modStatus True (modality_embed : Modality PROP1 PROP2)
      iprop(⎡P⎤ : PROP2) iprop(⎡P⎤) P where
  from_modal _ := .rfl

@[ipm_backtrack, rocq_alias from_modal_id_embed]
instance (priority := low) fromModal_id_embed {α} φ modStatus (sel : α)
    (P Q : PROP1) [inst : FromModal .known φ modality_id sel P Q] :
    FromModal (PROP1 := PROP2) modStatus φ modality_id sel iprop(⎡P⎤) iprop(⎡Q⎤) where
  from_modal h := embed_mono <| inst.from_modal h

@[ipm_backtrack, rocq_alias from_modal_affinely_embed]
instance (priority := low) fromModal_affinely_embed {α} φ modStatus (sel : α)
    (P Q : PROP1) [inst : FromModal .known φ modality_affinely sel P Q] :
    FromModal (PROP1 := PROP2) modStatus φ modality_affinely sel iprop(⎡P⎤) iprop(⎡Q⎤) where
  from_modal h := (embed_affinely_2 Q).trans (embed_mono <| inst.from_modal h)

@[ipm_backtrack, rocq_alias from_modal_persistently_embed]
instance (priority := low) fromModal_persistently_embed {α} φ modStatus (sel : α)
    (P Q : PROP1) [inst : FromModal .known φ modality_persistently sel P Q] :
    FromModal (PROP1 := PROP2) modStatus φ modality_persistently sel iprop(⎡P⎤) iprop(⎡Q⎤) where
...
instance (priority := low) fromModal_intuitionistically_embed {α} φ modStatus (sel : α)
    (P Q : PROP1) [inst : FromModal .known φ modality_intuitionistically sel P Q] :
    FromModal (PROP1 := PROP2) modStatus φ modality_intuitionistically sel iprop(⎡P⎤) iprop(⎡Q⎤) where
...
instance fromModal_later_embed [BiEmbedLater PROP1 PROP2] {α} φ modStatus (sel : α) n (P Q : PROP1)
    [inst : FromModal .known φ (modality_laterN n) sel P Q] :
    FromModal (PROP1 := PROP2) modStatus φ (modality_laterN n) sel iprop(⎡P⎤) iprop(⎡Q⎤) where
...
instance (priority := low) fromModal_plainly_embed {α} φ io (sel : α)
    (P Q : P1) [inst : FromModal .known φ modality_plainly sel P Q] :
