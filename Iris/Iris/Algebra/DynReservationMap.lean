...
instance instDiscreteEDynReservationMapMk {a : H A} [DiscreteE a] :
  ...
  discrete := fun h => (OFE.eq_dist (SI := Nat)).mpr <| by
    intro n
...
instance instUCMRADynReservationMap : UCMRA (DynReservationMap A H) where
  ...
  validN_op_left {n x y} v := by
    ...
    · cases (validN_disj v) i with
      ...
      | inr bb =>
        ...
        exact .inl HK
  assoc := (eq_dist (SI := Nat)).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.assoc.dist
  comm := (eq_dist (SI := Nat)).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.comm.dist
  pcore_op_left {x cx} h := (eq_dist (SI := Nat)).mpr <| by
    refine fun n => ⟨?_, ?_⟩
  ...
  pcore_op_mono {x cx} h y := by
    ...
    refine ⟨mk z w, (eq_dist (SI := Nat)).mpr ?_⟩
    refine fun n => ⟨?_, ?_⟩
  ...
  extend {n x y₁ y₂} v exy := by
    ...
    refine ⟨mk z₁ y₁.token, mk z₂ y₂.token, (eq_dist (SI := Nat)).mpr ?_, ⟨zy₁, rfl⟩, ⟨zy₂, rfl⟩⟩
  ...
  unit_left_id {x} := (OFE.eq_dist (SI := Nat)).mpr <| by
    exact fun n => ⟨(Algebra.MonoidOps.op_left_id : (∅ : H A) • x.data = x.data).dist,
      (pcore_op_left' rfl).dist⟩
  pcore_unit := (eq_dist (SI := Nat)).mpr <| by exact fun n => ⟨Heap.core_empty.dist, .rfl⟩
...
theorem included {x y : DynReservationMap A H} :
  ...
  exact fun ⟨⟨z₁, hz₁⟩, ⟨z₂, hz₂⟩⟩ =>
    ⟨mk z₁ z₂, (eq_dist (SI := Nat)).mpr (by exact fun n => ⟨hz₁.dist, hz₂.dist⟩)⟩
...
theorem alloc {e k} {a : A} (hke : k ∈ e) (va : ✓ a) :
  ...
  | some z =>
    ...
    rw [ze, assoc', ← (show mk ({[k := a]} • d) ∅ = mkData k a • mk d ∅ from
      (OFE.eq_dist (SI := Nat)).mpr <| by exact fun n => ⟨.rfl, Dist.of_eq (pcore_op_right_L rfl).symm⟩)]
...
theorem reserve (Q : DynReservationMap A H → Prop)
  ...
  have ⟨mf, Ef, hz, vmf, hinf, hdisj⟩ :
    ...
    match mz with
    | none =>
      exact ⟨∅, ∅, (OFE.eq_dist (SI := Nat)).mpr (by exact fun n =>
