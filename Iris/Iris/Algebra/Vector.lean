...
-- Rocq calls this instance `list_cofe`, clashing with the instance of the same name in
-- `iris/algebra/list.v`. The porting checker disambiguates duplicate short names by their
-- source file, so the two are aliased as `vector.list_cofe` and `list.list_cofe` respectively.
