...
public instance auth_ne : NonExpansive (auth (A := A)) where
...
public instance frag_ne : NonExpansive (frag (A := A)) where
...
public theorem valid {a : A} : ✓ (●E a) • ◯E a :=
  Auth.auth_both_valid_2 trivial .rfl

@[rocq_alias excl_auth_agreeN]
public theorem agreeN {a b : A} (h : ✓{n} (●E a) • ◯E b) : a ≡{n}≡ b :=
  dist_of_inc_exclusive (Auth.both_validN.mp h).1 trivial |>.symm

@[rocq_alias excl_auth_agree]
public theorem agree {a b : A} (h : ✓ (●E a) • ◯E b) : a ≡ b :=
  equiv_dist.mpr fun _ => agreeN (Valid.validN h)

@[rocq_alias excl_auth_agree_L]
public theorem agree_L [Leibniz A] {a b : A} (h : ✓ (●E a) • ◯E b) : a = b :=
  (agree h).to_eq

@[rocq_alias excl_auth_auth_op_validN]
public theorem auth_op_validN {a b : A} : (✓{n} (●E a) • ●E b) ↔ False :=
  Auth.auth_op_validN

@[rocq_alias excl_auth_auth_op_valid]
public theorem auth_op_valid {a b : A} : (✓ (●E a) • ●E b) ↔ False :=
  Auth.auth_op_valid

@[rocq_alias excl_auth_frag_op_validN]
public theorem frag_op_validN {a b : A} : (✓{n} (◯E a) • ◯E b) ↔ False := by
...
public theorem frag_op_valid {a b : A} : (✓ (◯E a) • ◯E b) ↔ False := by
...
public theorem update {a b a' : A} : ((●E a) • ◯E b) ~~> ((●E a') • ◯E a') :=
