...
theorem agree {dq : DFrac} {a b : A} (h : ✓ (●F{dq} a) • ◯F b) : a = b :=
  OFE.Equiv.to_eq <| equiv_dist.mpr fun n => agreeN (valid_iff_validN.mp h n)
...
instance isOp_frac_auth {q q1 q2 : Qp} {a1 a2 : A} {a : outParam A}
  ...
  is_op :=
    (NonExpansive.eqv (Equiv.of_eq (OFE.some_eqv_some.mpr
      (NonExpansive₂.eqv (OFE.Equiv.of_eq h1.is_op) (OFE.Equiv.of_eq h2.is_op)).to_eq))).to_eq
