...
theorem op_comm {x y : Agree α} : op x y = op y x :=
  OFE.eq_dist_2 (x.ind fun _ => y.ind fun _ => Raw.op_comm)
...
theorem op_assoc {x y z : Agree α} : op x (op y z) = op (op x y) z :=
  OFE.eq_dist_2 (x.ind fun _ => y.ind fun _ => z.ind fun _ => Raw.op_assoc)
...
theorem op_inv {x y : Agree α} : valid (op x y) → x = y :=
  x.ind fun _ => y.ind fun _ h => OFE.eq_dist_2 (Raw.op_inv h)
...
instance instCMRADiscrete [OFE.Discrete α] : CMRA.Discrete (Agree α) where
  discrete_0 {x y} := x.ind fun _ => y.ind fun _ h => OFE.eq_dist_2 (Raw.discrete_0 h)
