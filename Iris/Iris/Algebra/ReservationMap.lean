...
instance instDiscreteReservationMap [Discrete A] : Discrete (ReservationMap A H) where
  discrete_0 h := (OFE.eq_dist _).mpr <| by
    intro n
...
instance instDiscreteEReservationMapMk {a : H A} [DiscreteE a] :
  ...
  discrete := fun h => (OFE.eq_dist _).mpr <| by
    intro n
...
instance : UCMRA (ReservationMap A H) where
  ...
  validN_op_left {n x y} v := by
    ...
    · cases (validN_disj v) i with
      ...
      | inr bb =>
        ...
        exact .inl HK
  assoc := (OFE.eq_dist _).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.assoc.dist
  comm := (OFE.eq_dist _).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.comm.dist
  pcore_op_left {x cx} h := (OFE.eq_dist _).mpr <| by
    refine fun n => ⟨?_, ?_⟩
  ...
  pcore_idem {x cx} h := (OFE.eq_dist _).mpr <| by
    refine fun n => ⟨?_, ?_⟩
  ...
  pcore_op_mono {x cx} h y := by
    ...
    refine ⟨mk z w, (OFE.eq_dist _).mpr ?_⟩
    refine fun n => ⟨?_, ?_⟩
...
theorem singleton_op k (a b : A) :
  ...
  have he : (({[k := a]} : H A) • {[k := b]}) = {[k := a • b]} :=
    (OFE.eq_dist _).mpr fun n i => Dist.of_eq (Heap.singleton_op_singleton i)
