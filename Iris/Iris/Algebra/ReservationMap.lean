...
instance : UCMRA (ReservationMap A H) where
  ...
  validN_op_left {n x y} v := by
    ...
    · cases (validN_disj v) i with
      ...
      | inr bb =>
        ...
        exact .inl HK
  assoc := (OFE.eq_dist (SI := Nat)).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.assoc.dist
  comm := (OFE.eq_dist (SI := Nat)).mpr <| by refine fun _ => ⟨?_, ?_⟩ <;> exact CMRA.comm.dist
  pcore_op_left {x cx} h := (OFE.eq_dist (SI := Nat)).mpr <| by
    refine fun n => ⟨?_, ?_⟩
  ...
  pcore_idem {x cx} h := (OFE.eq_dist (SI := Nat)).mpr <| by
    refine fun n => ⟨?_, ?_⟩
  ...
  pcore_op_mono {x cx} h y := by
    ...
    refine ⟨mk z w, (OFE.eq_dist (SI := Nat)).mpr ?_⟩
    refine fun n => ⟨?_, ?_⟩
  ...
  extend {n x y₁ y₂} v exy := by
    ...
    refine ⟨mk z₁ y₁.token, mk z₂ y₂.token, (OFE.eq_dist (SI := Nat)).mpr ?_, ⟨zy₁, rfl⟩, ⟨zy₂, rfl⟩⟩
  ...
  pcore_unit := (OFE.eq_dist (SI := Nat)).mpr <| by exact fun n => ⟨Heap.core_empty.dist, .rfl⟩
...
instance instCoreIdSingleton {a : A} [CoreId a] : CoreId (singleton (H := H) k a) where
  core_id := (OFE.eq_dist (SI := Nat)).mpr <| by
...
theorem singleton_op k (a b : A) :
  ...
  have he : (({[k := a]} : H A) • {[k := b]}) = {[k := a • b]} :=
    (OFE.eq_dist (SI := Nat)).mpr fun n i => Dist.of_eq (Heap.singleton_op_singleton i)
