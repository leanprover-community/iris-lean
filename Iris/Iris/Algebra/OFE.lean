...
@[rocq_alias ofe]
class OFE (α : Type _) (SI : Type _ := by infer_stepindex) [SIdx SI] where
  Dist : SI → α → α → Prop
...
@[rocq_alias dist_lt]
theorem Dist.lt [OFE α] {m n : SI} {x y : α} : x ≡{n}≡ y → m < n → x ≡{m}≡ y := dist_lt
...
@[simp, refl] theorem Dist.rfl [OFE α] {n : SI} {x : α} : x ≡{n}≡ x := dist_eqv.1 _
@[symm] theorem Dist.symm [OFE α] {n : SI} {x : α} : x ≡{n}≡ y → y ≡{n}≡ x := dist_eqv.2
theorem Dist.trans [OFE α] {n : SI} {x : α} : x ≡{n}≡ y → y ≡{n}≡ z → x ≡{n}≡ z := dist_eqv.3
...
instance [OFE α] {n : SI} : Trans (OFE.Dist n) (OFE.Dist n) (OFE.Dist n : α → α → Prop) where
  trans := Dist.trans

/-- A function `f : α → β` is non-expansive if it preserves `n`-equivalence. -/
class NonExpansive {α β : Type _} (f : α → β) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] [OFE β] where
...
class NonExpansive₂ {α β γ : Type _} (f : α → β → γ) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] [OFE β] [OFE γ] where
...
theorem Dist.distLater [OFE α] {n : SI} {x y : α} (h : x ≡{n}≡ y) : DistLater n x y :=
  fun _ => dist_lt h

/-- `DistLater n`-equivalence implies `m`-equivalence for all `m < n`. -/
@[rocq_alias dist_later_dist_lt]
theorem DistLater.dist_lt [OFE α] {m n : SI} {x y : α} (h : DistLater n x y) (hm : m < n) : x ≡{m}≡ y :=
  h _ hm
...
theorem distLater_succ [OFE α] {n : SI} {x y : α} : DistLater (SIdx.succ n) x y ↔ x ≡{n}≡ y :=
  ⟨(·.dist_lt (SIdx.lt_succ_self _)), fun h1 _ h2 => h1.le (SIdx.lt_succ_r.mp h2)⟩

theorem distLater_soundness [OFE α] {x y : α} (H : ∀ n : SI, DistLater n x y → x ≡{n}≡ y) : x = y := by
...
@[rocq_alias Discrete]
class DiscreteE {α : Type _} (x : α) (SI : Type _ := by infer_stepindex) [SIdx SI] [OFE α] : Prop where
  discrete : x ≡{(0 : SI)}≡ y → x = y

/-- A discrete OFE is one where equivalence is implied by `0`-equivalence. -/
@[rocq_alias OfeDiscrete]
class Discrete (α : Type _) (SI : Type _ := by infer_stepindex) [SIdx SI] [OFE α] where
...
@[rocq_alias discrete]
theorem Discrete.discrete [OFE α] [Discrete α] {n : SI} {x y : α} (h : x ≡{n}≡ y) : x = y :=
...
/-- For discrete OFEs, `n`-equivalence implies equivalence for any `n`. -/
theorem Discrete.discrete_n [OFE α] [Discrete α] {n : SI} {x y : α} (h : x ≡{(0 : SI)}≡ y) : x ≡{n}≡ y :=
  (discrete h).dist
...
theorem Discrete.discrete_iff_0 [OFE α] [Discrete α] (n : SI) {x y : α} : x ≡{(0 : SI)}≡ y ↔ x ≡{n}≡ y :=
...
non-expansive. -/
@[ext, rocq_alias ofe_mor] structure Hom (α β : Type _) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] [OFE β] where
  f : α → β
  ne : NonExpansive f
...
@[inherit_doc]
notation:25 α:26 " -n>@{" SI "} " β:25 => Hom α β SI
...
@[ext] structure ContractiveHom (α β : Type _) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] [OFE β] extends Hom α β SI where
  [contractive : Contractive f]
  ne := ne_of_contractive f

infixr:25 " -c> " => ContractiveHom

notation:25 α:26 " -c>@{" SI "} " β:25 => ContractiveHom α β SI

instance [OFE α] [OFE β] : CoeFun (α -c> β) (fun _ => α → β) := ⟨fun x => x.toHom.f⟩
instance [OFE α] [OFE β] (f : α -c> β) : Contractive f := f.contractive

def _root_.Function.toContractiveHom (f : α → β) [OFE α] [OFE β] [ι : OFE.Contractive f SI] : α -c> β where
  f := f
  contractive := ι

@[simp] theorem _root_.Function.toContractiveHom_apply {f : α → β} [OFE α] [OFE β] [ι : OFE.Contractive f SI] {x} :
  f.toContractiveHom (SI := SI) x = f x := by rfl
...
instance : @DiscreteE Unit (() : Unit) SI instSI (unitOFE ) :=
  letI := unitOFE (SI := SI)
...
@[simp] theorem some_eqv_some {α : Type _} {x y : α} : (some x = some y) ↔ x = y :=
  ⟨Option.some.inj, congrArg some⟩
@[simp] theorem not_some_eqv_none {α : Type _} {x : α} : ¬some x = none := Option.some_ne_none x
@[simp] theorem not_none_eqv_some {α : Type _} {x : α} : ¬none = some x := fun h => Option.some_ne_none x h.symm

@[simp, rocq_alias dist_Some]
theorem some_dist_some [OFE α] {n : SI} {x y : α} : (some x ≡{n}≡ some y) ↔ x ≡{n}≡ y := .rfl
@[simp] theorem not_some_dist_none [OFE α] {n : SI} {x : α} : ¬some x ≡{n}≡ none := id
@[simp] theorem not_none_dist_some [OFE α] {n : SI} {x : α} : ¬none ≡{n}≡ some x := id

theorem equiv_some {α : Type _} {o : Option α} {y : α} (e : o = some y) :
    ∃ z, o = some z ∧ z = y := ⟨y, e, rfl⟩

theorem equiv_none {α : Type _} {o : Option α} : o = none ↔ o = none := Iff.rfl
...
abbrev OFEFun {α : Type _} (β : α → Type _) := ∀ a, OFE (β a) SI
...
theorem equiv_fst {α β : Type _} {x y : α × β} (h : x = y) : x.fst = y.fst := congrArg Prod.fst h
theorem equiv_snd {α β : Type _} {x y : α × β} (h : x = y) : x.snd = y.snd := congrArg Prod.snd h
theorem equiv_prod_ext {α β : Type _} {x₁ x₂ : α} {y₁ y₂ : β}
    (ex : x₁ = x₂) (ey : y₁ = y₂) : (x₁, y₁) = (x₂, y₂) := by subst ex; subst ey; rfl

theorem dist_fst {n : SI} [OFE α] [OFE β] {x y : α × β} (h : x ≡{n}≡ y) : x.fst ≡{n}≡ y.fst := h.left
theorem dist_snd {n : SI} [OFE α] [OFE β] {x y : α × β} (h : x ≡{n}≡ y) : x.snd ≡{n}≡ y.snd := h.right

@[rocq_alias pair_dist]
theorem dist_prod_ext {n : SI} [OFE α] [OFE β] {x₁ x₂ : α} {y₁ y₂ : β}
...
instance : OFE (α ⊕ β) where
  Dist n
    | .inl a, .inl b => Dist n a b
    | .inr a, .inr b => Dist n a b
    | _, _ => False
  dist_eqv := {
    refl
...
instance instOFESigma (P : α → Type _) [∀ x, OFE (P x) SI] : OFE (Sigma P) where
  Dist n x y := ∃ heq : x.fst = y.fst, Dist n (heq ▸ x.snd) y.snd
  dist_eqv := {
    refl _ := ⟨rfl, .rfl⟩
...
instance instDiscreteESigma {P : α → Type _} [∀ x, OFE (P x) SI] {x : Sigma P} [inst : DiscreteE x.snd] :
    DiscreteE x where
  discrete {y} := by
...
instance instDiscreteSigma {P : α → Type _} [∀ x, OFE (P x) SI] [∀ x, Discrete (P x) SI] :
...
theorem Sigma.equiv_eq_alt {P : α → Type _} [∀ x, OFE (P x) SI] {x1 x2 : Sigma P} :
...
instance Sigma.fst_ne {P : α → Type _} [OFE α] [∀ x, OFE (P x) SI] :
...
theorem Sigma.dist_snd {P : α → Type _} [∀ x, OFE (P x) SI] {n : SI} {x y : Sigma P}
    (h : Dist n x y) : h.1 ▸ x.snd ≡{n}≡ y.snd := h.2

@[rocq_alias projT2_proper]
theorem Sigma.equiv_snd {P : α → Type _} [∀ x, OFE (P x) SI] {x y : Sigma P}
    (h : x = y) : congrArg Sigma.fst h ▸ x.snd = y.snd := by
  subst h; rfl

@[rocq_alias existT_ne]
theorem Sigma.mk_dist {P : α → Type _} [∀ x, OFE (P x) SI] {n : SI} {i1 i2 : α} {v1 : P i1} {v2 : P i2}
    (heq : i1 = i2) (h : heq ▸ v1 ≡{n}≡ v2) : Sigma.mk i1 v1 ≡{n}≡ Sigma.mk i2 v2 :=
  ⟨heq, h⟩

@[rocq_alias existT_proper]
theorem Sigma.mk_equiv {P : α → Type _} [∀ x, OFE (P x) SI] {i1 i2 : α} {v1 : P i1} {v2 : P i2}
    (heq : i1 = i2) (h : heq ▸ v1 = v2) : Sigma.mk i1 v1 = Sigma.mk i2 v2 := by
  subst heq; subst h; rfl

@[rocq_alias existT_ne_2]
instance Sigma.mk_ne {P : α → Type _} [∀ x, OFE (P x) SI] (a : α) :
...
identity morphism. -/
@[ext, rocq_alias ofe_iso] structure Iso (α β : Type _) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] [OFE β] where
  hom : α -n> β
  inv : β -n> α
  hom_inv : hom (inv x) = x
...
@[simp] theorem Iso.hom_inv_dist [OFE α] [OFE β] (iso : Iso α β) {n : SI} {x} :
    iso.hom (iso.inv x) ≡{n}≡ x :=
  (Iso.hom_inv iso).dist

@[simp] theorem Iso.inv_hom_dist [OFE α] [OFE β] (iso : Iso α β) {n : SI} {x} :
...
theorem Iso.hom_dist [OFE α] [OFE β] (iso : Iso α β) {n : SI} ⦃x y⦄ :
...
theorem Iso.inv_dist [OFE α] [OFE β] (iso : Iso α β) {n : SI} ⦃x y⦄ :
...
`n`-equivalence. -/
@[rocq_alias chain] structure Chain (α : Type _) (SI : Type _ := by infer_stepindex)
    [SIdx SI] [OFE α] where
  chain : SI → α
...
@[simp] theorem map_apply [OFE α] [OFE β] {f : α -n> β} {c : Chain α} {n : SI} :
    map f c n = f (c n) := rfl
...
theorem chain_none_const [SIdx SI] [OFE V] {c : Chain (Option V)} (H : c n = none) :
...
theorem chain_option_some [SIdx SI] [OFE V] {c : Chain (Option V)} (H : c n = some v) :
...
@[rocq_alias Cofe]
class IsCOFE (α : Type _) (SI : Type _ := by infer_stepindex) [SIdx SI] [OFE α SI] where
  compl : Chain α → α
...
/-- Complete ordered family of equivalences -/
class abbrev COFE (α : Type _) (SI : Type _ := by infer_stepindex) [SIdx SI] := OFE α SI, IsCOFE α SI
...
theorem compl_const [COFE α] (a : α) : compl (Chain.const (SI := SI) a) = a :=
...
instance : @Discrete Unit SI _ unitOFE :=
...
instance {α : Type _} (β : α → Type _) [∀ x, COFE (β x) SI] : COFE ((x : α) → β x) where
...
theorem Sigma.chain_const_proj1 {P : α → Type _} [∀ x, OFE (P x) SI] [∀ x, IsCOFE (P x) SI]
    (c : Chain (Sigma P)) n : (c n).fst = (c (0 : SI)).fst :=
  (c.cauchy SIdx.le_0_l).choose

@[rocq_alias sigT_bchain_const_proj1]
theorem Sigma.bchain_const_proj1 {P : α → Type _} [∀ x, OFE (P x) SI]
...
theorem Sigma.dist_cast_of_dist {P : α → Type _} [∀ x, OFE (P x) SI] {n : SI}
...
def Sigma.bchain_map_snd {P : α → Type _} [∀ x, OFE (P x) SI]
...
theorem Sigma.lbcompl_cast {P : α → Type _} [∀ x, OFE (P x) SI] [∀ x, IsCOFE (P x) SI]
    {a b : α} (eq : a = b) {n : SI} (hn : SIdx.Limit n) (c : BChain (P a) n) :
    (eq ▸ IsCOFE.lbcompl hn c : P b)
      = IsCOFE.lbcompl hn (eq ▸ c : BChain (P b) n) := by
  subst eq; rfl

theorem Sigma.bchain_cast_apply {P : α → Type _} [∀ x, OFE (P x) SI] {a b : α} (eq : a = b)
...
def Sigma.chain_map_snd {P : α → Type _} [∀ x, OFE (P x) SI] [∀ x, IsCOFE (P x) SI] (c : Chain (Sigma P)) :
...
instance {P : α → Type _} [∀ x, OFE (P x) SI] [∀ x, IsCOFE (P x) SI] : IsCOFE (Sigma P) where
...
abbrev OFunctorPre (SI : Type _ := by infer_stepindex) [SIdx SI] := ∀ α β [COFE α SI] [COFE β SI], Type _
...
@[reducible]
def DiscreteO.instCOFE [SIdx SI] {α : Type _} : COFE (DiscreteO α) := COFE.ofDiscrete _

theorem DiscreteO.OFE [SIdx SI] {α : Type _} :
    @OFE.Discrete (DiscreteO α) SI _ (OFE.ofDiscrete _) :=
  letI : Iris.OFE (DiscreteO α) := Iris.OFE.ofDiscrete _
  ⟨id⟩
...
theorem Option.map_ne [OFE β] {f g : α → β} {x y : Option α} {n : SI} :
...
variable [SIdx SI] [OFE A] [OFE A'] [OFE B] [OFE B']
...
omit [OFE A] [OFE A'] [OFE B] [OFE B'] in
...
@[rocq_alias sigT_map]
def Sigma.mapO {P1 P2 : A → Type _} [∀ x, OFE (P1 x) SI] [∀ x, OFE (P2 x) SI] :
...
class LimitPreserving {α : Type _} (P : α → Prop) (SI : Type _ := by infer_stepindex) [SIdx SI] [COFE α] : Prop where
...
theorem LimitPreserving.discrete [COFE α] {P : α → Prop}
    (hdiscrete : ∀ {x y : α}, x ≡{(0 : SI)}≡ y → (P x → P y)) : LimitPreserving P where
...
nonrec abbrev OFE.ContractiveHom.fixpoint [COFE α] [Inhabited α] (f : α -c> α) : α := fixpoint (SI := SI) f.f
...
instance OFE.ContractiveHom.fixpoint_ne [COFE α] [Inhabited α] :
  ...
  ne n f1 f2 h := by
    ...
    | h n ih =>
      intro h
      exact (fixpoint_unfold f1).dist.trans <| (h _).trans <|
        (Contractive.distLater_dist fun p Hp => ih p Hp (h.lt Hp)).trans
          (fixpoint_unfold f2).dist.symm
...
theorem fixpointAA_contractive [COFE α] [COFE β] [Inhabited α] [Inhabited β]
    (fA : α -c>@{SI} β -n>@{SI} α) (fB : α -c>@{SI} β -c>@{SI} β) : Contractive (fixpointAA fA fB) where
...
def laterLimitBChain [OFE A] {n : SI} (c : BChain (Later A) n) (hn : SIdx.Limit n) :
    BChain A n where
...
theorem OFE.cast_dist [SIdx SI] [Iα : OFE α] [Iβ : OFE β] {x y : α}
