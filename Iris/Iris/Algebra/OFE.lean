...
theorem dist_equivalence [OFE SI α] {n : SI} : Equivalence (Dist (α := α) n) := dist_eqv

@[rocq_alias dist_lt]
theorem Dist.lt [OFE SI α] {m n : SI} {x y : α} : x ≡{n}≡ y → m < n → x ≡{m}≡ y := dist_lt
...
@[simp, refl] theorem Dist.rfl [OFE SI α] {n : SI} {x : α} : x ≡{n}≡ x := dist_eqv.1 _
@[symm] theorem Dist.symm [OFE SI α] {n : SI} {x : α} : x ≡{n}≡ y → y ≡{n}≡ x := dist_eqv.2
theorem Dist.trans [OFE SI α] {n : SI} {x : α} : x ≡{n}≡ y → y ≡{n}≡ z → x ≡{n}≡ z := dist_eqv.3
theorem Dist.of_eq [OFE SI α] {x y : α} : x = y → x ≡{n}≡ y := (· ▸ .rfl)

#rocq_ignore ofe_equivalence "OFE is Leibniz; use equality"
theorem _root_.Eq.dist [OFE SI α] {x y : α} (h : x = y) : x ≡{n}≡ y := h ▸ .rfl

instance [OFE SI α] {n : SI} : Trans (OFE.Dist n) (OFE.Dist n) (OFE.Dist n : α → α → Prop) where
  trans := Dist.trans
...
instance id_ne [OFE SI α] : NonExpansive (@id α) := ⟨fun _ _ _ h => h⟩

instance const_ne [OFE SI α] [OFE SI β] {x : α} : NonExpansive (Function.const β x) := ⟨fun _ _ _ _ => .rfl⟩

/-- Note: Not an instance, as any function can be decomposed as a composition in multiple ways. -/
theorem NonExpansive.comp [OFE SI α] [OFE SI β] [OFE SI γ] {g : β → γ} {f : α → β}
...
theorem NonExpansive₂.ne_right [OFE SI α] [OFE SI β] [OFE SI γ] (f : α → β → γ) [NonExpansive₂ f]
    (a : α) : NonExpansive (f a) :=
  ⟨fun {_ _ _} h => ne Dist.rfl h⟩

/-- Note: Not an instance, due to instance coherence problems. -/
theorem NonExpansive₂.ne_left [OFE SI α] [OFE SI β] [OFE SI γ] (f : α → β → γ) [NonExpansive₂ f]
...
def DistLater [OFE SI α] (n : SI) (x y : α) : Prop := ∀ m, m < n → x ≡{m}≡ y
...
@[simp, rocq_alias contractive_0] theorem Contractive.zero [OFE SI α] [OFE SI β] (f : α → β)
    [Contractive f] {x y} : f x ≡{0}≡ f y :=
  Contractive.distLater_dist distLater_zero

@[rocq_alias contractive_S]
theorem Contractive.succ [OFE SI α] [OFE SI β] (f : α → β) [Contractive f] {n x y}
...
instance ne_of_contractive [OFE SI α] [OFE SI β] (f : α → β) [Contractive f] : NonExpansive f where
...
instance [OFE SI α] [OFE SI β] {x : β} : Contractive (fun _ : α => x) where
...
def ofDiscrete (α : Type _) : OFE SI α where
  Dist _ := Eq
...
theorem discreteE_eqv [OFE SI α] {x y : α} (h : x = y) : DiscreteE x ↔ DiscreteE y := h ▸ Iff.rfl
...
/-- For discrete OFEs, `n`-equivalence implies equivalence for any `n`. -/
theorem Discrete.discrete_n [OFE SI α] [Discrete α] {n : SI} {x y : α} (h : x ≡{0}≡ y) : x ≡{n}≡ y :=
  (discrete h).dist
export OFE.Discrete (discrete_n)

@[rocq_alias discrete_iff]
theorem Discrete.discrete_iff [OFE SI α] [Discrete α] (n) {x y : α} : x = y ↔ x ≡{n}≡ y :=
  ⟨Eq.dist, discrete⟩
...
@[reducible] def mkQuotient {X : Type u} (dist : SI → X → X → Prop)
    ...
    OFE SI (Quotient (QuotientO dist heqv)) :=
...
@[reducible] def map {X' : Type u'} {dist' : SI → X' → X' → Prop} {heqv' : ∀ {n}, Equivalence (dist' n)}
...
instance [OFE SI α] [OFE SI β] : CoeFun (α -n> β) (fun _ => α → β) := ⟨Hom.f⟩
instance [OFE SI α] [OFE SI β] (f : α -n> β) : NonExpansive f := f.ne

/-- The identity morphism on an OFE. -/
@[rocq_alias cid]
protected def Hom.id [OFE SI α] : α -n> α where
  f := id
  ne.ne _ _ _ := id

/-- The composition of two morphisms between OFEs. -/
@[rocq_alias ccompose]
protected def Hom.comp [OFE SI α] [OFE SI β] [OFE SI γ] (g : β -n> γ) (f : α -n> β) : α -n> γ where
  f := g.f ∘ f.f
...
@[simp] theorem Hom.id_apply [OFE SI α] {x} : (Hom.id : α -n> α) x = x := rfl
@[simp] theorem Hom.comp_apply [OFE SI α] [OFE SI β] [OFE SI γ] {g : β -n> γ} {f : α -n> β} {x} :
    (g.comp f) x = g (f x) := rfl

@[simp] theorem Hom.id_comp [OFE SI α] [OFE SI β] {f : α -n> β} : Hom.id.comp f = f := rfl
@[simp] theorem Hom.comp_id [OFE SI α] [OFE SI β] {f : α -n> β} : f.comp Hom.id = f := rfl

theorem Hom.comp_assoc [OFE SI α] [OFE SI β] [OFE SI γ] [OFE SI δ]
...
def Hom.ofSubtype [OFE SI α] [OFE SI β] (f : { f : α → β // NonExpansive f }) : α -n> β :=
  ⟨f.val, f.property⟩

@[ext] structure ContractiveHom (α β : Type _) [OFE SI α] [OFE SI β] extends Hom α β where
  [contractive : Contractive f]
  ne := ne_of_contractive f

infixr:25 " -c> " => ContractiveHom

instance [OFE SI α] [OFE SI β] : CoeFun (α -c> β) (fun _ => α → β) := ⟨fun x => x.toHom.f⟩
instance [OFE SI α] [OFE SI β] (f : α -c> β) : Contractive f := f.contractive

def _root_.Function.toContractiveHom (f : α → β) [OFE SI α] [OFE SI β] [ι : OFE.Contractive f] : α -c> β where
  f := f
  contractive := ι

@[simp] theorem _root_.Function.toContractiveHom_apply {f : α → β} [OFE SI α] [OFE SI β] [ι : OFE.Contractive f] {x} :
...
def uliftUpHom [OFE SI α] : α -n> ULift α where
  f := .up
  ne.1 _ _ _ := id

def uliftDownHom [OFE SI α] : ULift α -n> α where
  f := ULift.down
  ne.1 _ _ _ := id
...
instance [OFE SI α] : OFE SI (Option α) where
...
instance [OFE SI α] [OFE.Discrete α] : OFE.Discrete (Option α) where
  discrete_0 {mx my} e :=
    match mx, my with
    | none,   none   => rfl
    | none,   some _ => e.elim
...
theorem dist_none [OFE SI α] {o : Option α} : o ≡{n}≡ none ↔ o = none :=
  ⟨fun h => by cases o with | none => rfl | some _ => exact h.elim, (· ▸ .rfl)⟩

@[rocq_alias dist_Some_inv_r']
theorem dist_some [OFE SI α] {n mx y} (h : mx ≡{n}≡ some y) :
    ∃ z : α, mx = some z ∧ y ≡{n}≡ z :=
...
instance [OFE SI α] [Discrete α] : Discrete (Option α) where
  discrete_0 {x y} H :=
    match x, y with
    | none, none => rfl
...
@[rocq_alias Some_ne]
instance OFE.Option.some.ne [OFE SI α] : OFE.NonExpansive (some : α → Option α) := ⟨fun _ _ _ => id⟩

@[rocq_alias Some_discrete]
instance Option.some_is_discrete [OFE SI α] {e : α} [OFE.DiscreteE e] : OFE.DiscreteE (some e) where
  discrete {y} h :=
    match y with
...
/-- Note: Not an instance, due to instance coherence problems. -/
theorem Option.ne_match [OFE SI α] {B : Type _} [OFE SI B]
...
instance Option.none_is_discrete [OFE SI α] : DiscreteE (none : Option α) := by
  constructor; rintro (_|_) <;> simp

instance Option.merge_ne [OFE SI α] {op : α → α → α} [NonExpansive₂ op] :
...
instance Option.bind_fun_ne [OFE SI α] [OFE SI β] (f : α → Option β) [NonExpansive f] : NonExpansive (flip Option.bind f) where
...
theorem Option.bind_dist [OFE SI α] [OFE SI β] {x : Option α} {f g : α → Option β} (H : ∀ x, f x ≡{n}≡ g x) : Option.bind x f ≡{n}≡ Option.bind x g :=
  match x with
  | some _ => H _
  | none => .rfl
...
instance [OFEFun (SI := SI) (β : α → _)] : OFE SI ((x : α) → β x) where
...
instance [OFE SI α] [OFE SI β] : OFE SI (α -n> β) where
...
instance ofe_mor_car_ne [OFE SI α] [OFE SI β] :
...
theorem ofe_mor_car_proper [OFE SI α] [OFE SI β] ⦃f g : α -n> β⦄ (hfg : f = g)
    ⦃x y : α⦄ (hxy : x = y) : f x = g y := by
  subst hfg; subst hxy; rfl

@[rocq_alias ofe_mor_inhabited]
instance [OFE SI α] [OFE SI β] [Inhabited β] : Inhabited (α -n> β) where
  default := { f := Function.const α default, ne := const_ne }

instance [OFE SI α] [OFE SI β] : OFE SI (α -c> β) where
...
def applyNe [OFE SI α] [OFE SI β] (x : α) : (α -n> β) -n> β where
  f f := f x
  ne.1 _ _ _ H := H x

instance [OFE SI α] [OFE SI β] : NonExpansive (applyNe (α := α) (β := β)) where
...
instance [OFE SI α] [OFE SI β] : OFE SI (α × β) where
...
theorem dist_snd {n : SI} [OFE SI α] [OFE SI β] {x y : α × β} (h : x ≡{n}≡ y) : x.snd ≡{n}≡ y.snd := h.right

@[rocq_alias pair_dist]
theorem dist_prod_ext {n : SI} [OFE SI α] [OFE SI β] {x₁ x₂ : α} {y₁ y₂ : β}
    (ex : x₁ ≡{n}≡ x₂) (ey : y₁ ≡{n}≡ y₂) : (x₁, y₁) ≡{n}≡ (x₂, y₂) := ⟨ex, ey⟩

@[rocq_alias pair_ne]
instance Prod.mk_ne [OFE SI α] [OFE SI β] : NonExpansive₂ (Prod.mk (α := α) (β := β)) where
  ne _ _ _ hx _ _ hy := dist_prod_ext hx hy

/-- Note: Not an instance, due to instance coherence problems. -/
theorem prod_mk_ne_left [OFE SI α] [OFE SI β] (b : β) : NonExpansive (β := α × β) (·, b) :=
  ⟨fun {_ _ _} h => dist_prod_ext h Dist.rfl⟩

/-- Note: Not an instance, due to instance coherence problems. -/
theorem prod_mk_ne_right [OFE SI α] [OFE SI β] (a : α) : NonExpansive (β := α × β) (a, ·) :=
  ⟨fun {_ _ _} h => dist_prod_ext Dist.rfl h⟩

@[rocq_alias fst_ne]
instance [OFE SI α] [OFE SI β] : NonExpansive (Prod.fst (α := α) (β := β)) :=
  ⟨fun {_ _ _} h => dist_fst h⟩

@[rocq_alias snd_ne]
instance [OFE SI α] [OFE SI β] : NonExpansive (Prod.snd (α := α) (β := β)) :=
...
instance prod.is_discrete [OFE SI α] [OFE SI β] {a : α} {b : β} [DiscreteE a] [DiscreteE b] :
    DiscreteE (a, b) := by
  constructor
  intro y H; exact Prod.ext (DiscreteE.discrete H.1) (DiscreteE.discrete H.2)

@[rocq_alias prod_ofe_discrete]
instance instDiscreteProd [OFE SI α] [OFE SI β] [Discrete α] [Discrete β] : Discrete (α × β) where
...
instance instNonExpansiveElim [OFE SI γ] {f₁ : α → γ} {f₂ : β → γ} [NonExpansive f₁] [NonExpansive f₂] :
...
instance [OFE SI α] (P : α → Prop) : OFE SI (Subtype P) where
...
instance Hom.ofSubtype_ne [OFE SI α] [OFE SI β] : NonExpansive (Hom.ofSubtype (α := α) (β := β)) :=
  ⟨fun {_ _ _} h => h⟩

/-- Extract the underlying subtype from a `Hom`. -/
def Hom.toSubtype [OFE SI α] [OFE SI β] (f : α -n> β) : { f : α → β // NonExpansive f } :=
  ⟨f.f, f.ne⟩

instance Hom.toSubtype_ne [OFE SI α] [OFE SI β] : NonExpansive (Hom.toSubtype (α := α) (β := β)) :=
  ⟨fun {_ _ _} h => h⟩

@[rocq_alias sigT_ofe_mixin]
instance instOFESigma (P : α → Type _) [∀ x, OFE SI (P x)] : OFE SI (Sigma P) where
...
instance instDiscreteESigma {P : α → Type _} [∀ x, OFE SI (P x)] {x : Sigma P} [inst : DiscreteE x.snd] :
    DiscreteE x where
  discrete {y} := by
...
instance instDiscreteSigma {P : α → Type _} [∀ x, OFE SI (P x)] [∀ x, Discrete (P x)] :
...
instance Sigma.fst_ne {P : α → Type _} [OFE SI α] [∀ x, OFE SI (P x)] :
...
theorem Sigma.mk_dist {P : α → Type _} [∀ x, OFE SI (P x)] {n : SI} {i1 i2 : α} {v1 : P i1} {v2 : P i2}
...
instance Sigma.mk_ne {P : α → Type _} [∀ x, OFE SI (P x)] (a : α) :
...
instance [OFE SI α] [OFE SI β] : CoeFun (Iso α β) (fun _ => α -n> β) := ⟨Iso.hom⟩
instance [OFE SI α] [OFE SI β] (iso : Iso α β) : NonExpansive iso.hom := iso.hom.ne
instance [OFE SI α] [OFE SI β] (iso : Iso α β) : NonExpansive iso.inv := iso.inv.ne

@[simp] theorem Iso.hom_inv_dist [OFE SI α] [OFE SI β] (iso : Iso α β) {n : SI} {x} :
    iso.hom (iso.inv x) ≡{n}≡ x :=
  (Iso.hom_inv iso).dist

@[simp] theorem Iso.inv_hom_dist [OFE SI α] [OFE SI β] (iso : Iso α β) {n : SI} {x} :
    iso.inv (iso.hom x) ≡{n}≡ x :=
  (Iso.inv_hom iso).dist

/-- OFE isomorphisms preserve equivalence. -/
theorem Iso.hom_eqv [OFE SI α] [OFE SI β] (iso : Iso α β) ⦃x y⦄ :
...
theorem Iso.inv_eqv [OFE SI α] [OFE SI β] (iso : Iso α β) ⦃x y⦄ :
...
theorem Iso.hom_dist [OFE SI α] [OFE SI β] (iso : Iso α β) {n : SI} ⦃x y⦄ :
...
theorem Iso.inv_dist [OFE SI α] [OFE SI β] (iso : Iso α β) {n : SI} ⦃x y⦄ :
...
def Iso.id [OFE SI α] : Iso α α where
  hom := Hom.id
  inv := Hom.id
  hom_inv := by intro x; simp
  inv_hom := by intro x; simp

@[simp] theorem Iso.id_apply [OFE SI α] {x} : ((Iso.id : Iso α α) : α -n> α) x = x := rfl

/-- The inverse of an OFE isomorphism -/
@[rocq_alias iso_ofe_sym]
def Iso.symm [OFE SI α] [OFE SI β] (iso : Iso α β) : Iso β α where
  hom := iso.inv
  inv := iso.hom
  hom_inv := by intro x; simp
  inv_hom := by intro x; simp
...
def Iso.comp [OFE SI α] [OFE SI β] [OFE SI γ] (iso1 : Iso β γ) (iso2 : Iso α β) : Iso α γ where
...
`n`-equivalence. -/
@[rocq_alias chain] structure Chain {SI} (α : Type _) [SIdx SI] [OFE SI α] where
  chain : SI → α
  cauchy : n ≤ i → chain i ≡{n}≡ chain n

instance [SIdx SI] [OFE SI α] : CoeFun (Chain α) (fun _ => SI → α) := ⟨Chain.chain⟩
...
def const [OFE SI α] (a : α) : Chain α where
  chain := fun _ => a
  cauchy _ := OFE.Dist.rfl

@[simp] theorem const_apply [OFE SI α] {a : α} {n : SI} : const a n = a := rfl

/-- Mapping a chain through a non-expansive function. -/
@[rocq_alias chain_map]
def map [OFE SI α] [OFE SI β] (f : α -n> β) (c : Chain α) : Chain β where
  chain n := f (c n)
  cauchy h := f.ne.1 (c.cauchy h)

@[simp] theorem map_apply [OFE SI α] [OFE SI β] {f : α -n> β} {c : Chain α} {n : SI} :
    map f c n = f (c n) := rfl

@[simp] theorem map_id [OFE SI α] {c : Chain α} : map (Hom.id : α -n> α) c = c := by
  simp [map]

theorem map_comp [OFE SI α] [OFE SI β] [OFE SI γ] {f : α -n> β} {g : β -n> γ} {c : Chain α} :
...
theorem chain_none_const [SIdx SI] [OFE SI V] {c : Chain (Option V)} (H : c n = none) :
...
theorem chain_option_some [SIdx SI] [OFE SI V] {c : Chain (Option V)} (H : c n = some v) :
...
theorem conv_compl' [COFE SI α] {c : Chain α} {n i} (h : n ≤ i) : compl c ≡{n}≡ c i :=
  conv_compl.trans (c.cauchy h).symm

/-- Chain maps commute with completion. -/
@[rocq_alias compl_chain_map]
theorem compl_map [COFE SI α] [COFE SI β] (f : α -n> β) (c : Chain α) :
...
theorem compl_const [COFE SI α] (a : α) : compl (Chain.const a) = a :=
  OFE.eq_dist.mpr (fun _ => conv_compl)

/-- Completion of discrete COFEs is the constant value. -/
@[simp] theorem discrete_cofe_compl [COFE SI α] [OFE.Discrete α] (c : Chain α) : compl c = c 0 :=
...
def ofDiscrete (α : Type _) : COFE SI α :=
  letI : OFE SI α := OFE.ofDiscrete α
  {
    compl c := c 0
...
instance instIsCOFEHom [OFE SI α] [OFE SI β] [IsCOFE SI β] : IsCOFE SI (α -n> β) where
  compl c := by
...
theorem Sigma.bchain_const_proj1 {P : α → Type _} [∀ x, OFE SI (P x)]
...
theorem Sigma.bchain_cast_apply {P : α → Type _} [∀ x, OFE SI (P x)] {a b : α} (eq : a = b)
...
abbrev OFunctorPre (SI : outParam <| Type _) [SIdx SI] := ∀ α β [COFE SI α] [COFE SI β], Type _
...
theorem DiscreteO.dist_inj [SIdx SI] {α : Type _} {x y : α} {n : SI} :
    letI : COFE SI (DiscreteO α) := DiscreteO.instCOFE
...
theorem Option.map_forall₂ {α β : Type _} [OFE SI α] [OFE SI β] (f : α → β)
    {o1 o2 : Option α} (h : o1 = o2) : o1.map f = o2.map f := congrArg (Option.map f) h

@[rocq_alias optionO_map_ne]
instance optionMap_ne [OFE SI β] : NonExpansive (optionMap (α := α) (β := β)) where
  ne _ f _ h o :=
    Option.map_ne (fun _ _ hab => dist_eqv.trans (f.ne.ne hab) (h _)) (dist_eqv.refl o)

@[rocq_alias option_mbind_ne]
theorem Option.bind_ne [OFE SI β] {f g : α → Option β} {x y : Option α} {n}
...
variable [SIdx SI] [OFE SI A] [OFE SI A'] [OFE SI B] [OFE SI B']
...
@[rocq_alias sigT_map]
def Sigma.mapO {P1 P2 : A → Type _} [∀ x, OFE SI (P1 x)] [∀ x, OFE SI (P2 x)] :
...
@[rocq_alias constOF]
instance oFunctorConstOF [COFE SI B] : OFunctor SI (constOF B) where
  ofe := _
  map _ _ := ⟨id, id_ne⟩
...
instance OFunctor.constOF_contractive [COFE SI B] : OFunctorContractive SI (constOF B) where
...
abbrev HomOF (F1 F2 : OFunctorPre SI) [OFunctor SI F1] [OFunctor SI F2] : OFunctorPre SI :=
  fun (A : Type _) (B : Type _) (_ : COFE SI A) (_ : COFE SI B) => @F1 B A _ _ -n> @F2 A B _ _
...
instance instOFunctorHomOF [OFunctor SI F1] [OFunctor SI F2] : OFunctor SI (HomOF F1 F2) where
  ...
  map_id x := OFE.eq_dist.mpr fun _ a => ((map_id _).trans (congrArg x.f (map_id a))).dist
  map_comp _ _ _ _ x := OFE.eq_dist.mpr fun _ _ =>
...
def fixpointAux [COFE SI α] [Inhabited α] (f : α → α) [Contractive f] : α :=
  COFE.compl <| Fixpoint.chain f

theorem fixpointAux_unfold [COFE SI α] [Inhabited α] (f : α -c> α) :
...
opaque fixpointP [COFE SI α] [Inhabited α] (f : α → α) [Contractive f] : { x : α // x = f x } :=
  ⟨fixpointAux f, fixpointAux_unfold f.toContractiveHom⟩

/-- Fixpoints inside of a COFE -/
@[rocq_alias fixpoint]
def fixpoint [COFE SI α] [Inhabited α] (f : α → α) [Contractive f] : α :=
  (fixpointP f).val
...
nonrec abbrev OFE.ContractiveHom.fixpoint [COFE SI α] [Inhabited α] (f : α -c> α) : α := fixpoint f.f

@[rocq_alias fixpoint_unfold]
theorem fixpoint_unfold [COFE SI α] [Inhabited α] (f : α -c> α) :
...
instance [OFE SI α] [OFE SI β] [OFE SI γ] : CoeFun (α -c> β -n> γ) (fun _ => α → β → γ) := ⟨fun f x => (f.f x).f⟩
instance [OFE SI α] [OFE SI β] [OFE SI γ] : CoeFun (α -c> β -c> γ) (fun _ => α → β → γ) := ⟨fun f x => (f.f x).f⟩

/-- A Contractive function with NonExpansive function codomain is NonExpansive₂. -/
instance ne₂_of_contractive_ne [OFE SI α] [OFE SI β] [OFE SI γ] (fA : α -c> β -n> γ) : NonExpansive₂ fA where
...
instance ne₂_of_contractive [OFE SI α] [OFE SI β] [OFE SI γ] (fB : α -c> β -c> γ) : NonExpansive₂ fB where
  ne n x₁ x₂ Hx y₁ y₂ Hy := by
    refine .trans ?_ ((fB.f x₂).ne.ne Hy)
    apply fB.ne.ne Hx

@[rocq_alias fixpoint_AB]
def fixpointAB [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] (fB : α -c> β -c> β) (x : α) : β := by
  let con_hom : β -c> β := {
    f := fB x,
...
theorem fixpointAB_contractive [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] (fB : α -c> β -c> β) :
...
def fixpointAA [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] (fA : α -c> β -n> α)
    (fB : α -c> β -c> β) (x : α) : α :=
...
def fixpointA [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] (fA : α -c> β -n> α)
    (fB : α -c> β -c> β) : α := by
  let con_hom : α -c> α := {
    f := fixpointAA fA fB,
...
def fixpointB [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β]
    (fA : α -c> β -n> α) (fB : α -c> β -c> β) : β :=
  fixpointAB fB <| fixpointA fA fB

@[rocq_alias fixpoint_A_unfold]
theorem fixpointA_unfold [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β]
...
theorem fixpointB_unfold [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β]
    (fA : α -c> β -n> α) (fB : α -c> β -c> β) :
    fB (fixpointA fA fB) (fixpointB fA fB) = (fixpointB fA fB) := by
  exact .symm (fixpoint_unfold _)

@[rocq_alias fixpoint_A_unique]
theorem fixpointA_unique [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β]
...
theorem fixpointB_unique [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β]
    (fA : α -c> β -n> α) (fB : α -c> β -c> β) (Hp : fA p q = p) (Hq : fB p q = q) :
...
instance fixpointA_ne [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] :
...
instance fixpointB_ne [COFE SI α] [COFE SI β] [Inhabited α] [Inhabited β] :
...
instance NextContractive {A : Type _} [OFE SI A] : Contractive (@Later.next A) where
  distLater_dist := id
...
theorem OFE.cast_dist [SIdx SI] [Iα : OFE SI α] [Iβ : OFE SI β] {x y : α}
