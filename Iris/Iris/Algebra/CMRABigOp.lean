...
theorem bigOpL_none {f : Nat → A → Option M} {l : List A} :
  ...
  | cons a l ih =>
    rw [BigOpL.bigOpL_cons, Iris.Option.op_none_iff, ih]
...
theorem bigOpM_none {M' : Type _ → Type _} [LawfulFiniteMap M' K] {f : K → V → Option M}
    {m : M' V} :
    ([^ CMRA.op map] k ↦ x ∈ m, f k x) = none ↔
      ∀ k x, PartialMap.get? m k = some x → f k x = none := by
...
theorem bigOpMS_none [LawfulFiniteMultiSet MS A] {f : A → Option M} {X : MS} :
