...
class lock (GF : BundledGFunctors) [IrisGS_gen hlc Exp GF] where
  ...
  is_lock_persistent {N} γ lk (R : IProp GF) : Persistent (is_lock N γ lk R)
  is_lock_iff {N} γ lk (R₁ R₂ : IProp GF) : is_lock N γ lk R₁ ⊢ (▷ □ (R₁ ∗-∗ R₂)) -∗ is_lock N γ lk R₂
...
instance lock_name_inhabited [IrisGS_gen hlc Exp GF] [lk : lock GF] : Inhabited lk.name :=
...
instance is_lock_persistent_inst γ v R : Persistent (lk.is_lock N γ v R) :=
  lk.is_lock_persistent γ v R

instance locked_timeless_inst γ : Timeless (lk.locked N γ) :=
...
instance is_lock_ne γ v : OFE.NonExpansive (lk.is_lock N γ v) :=
