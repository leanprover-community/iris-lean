/-
Adapted from `Mathlib.Tactic.Linter.Whitespace`
...
```
-/
public register_option linter.iris.style.whitespace : Bool := {
  defValue := false
  descr := "enable the whitespace linter"
}

/-- If the `linter.iris.style.whitespace.verbose` option is `true`, the `whitespace` linter
reports some helpful diagnostic information. -/
public register_option linter.iris.style.whitespace.verbose : Bool := {
  defValue := false
...
def whitespaceLinter : Linter where run := withSetOptionIn fun stx ↦ do
  ...
  if let some pos := stx.getPos? then
    ...
    if colStart ≠ 0 then
      Linter.logLint linter.iris.style.whitespace stx
  ...
  let fmt : Option Format ←
      try
        ...
        Linter.logLintIf linter.iris.style.whitespace.verbose (stx.getHead?.getD stx)
  ...
  if let some fmt := fmt then
    ...
    for s in scan do
      ...
      if s.msg.startsWith "Oh no" then
        Linter.logLintIf linter.iris.style.whitespace.verbose (.ofRange rg)
      ...
      Linter.logLint linter.iris.style.whitespace (.ofRange rg)
