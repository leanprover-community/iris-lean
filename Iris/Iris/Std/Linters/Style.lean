...
* names starting with `Parser` or containing a `Simps` component
...
* names that end in `_<number>` (these tend to be autogenerated instance names),
* library notes.
-/
public def isBadNameWithUnderscore (name : Name) : Bool := Id.run do
  ...
  if last.endsWith '_' ||
      ...
      (`Parser).isPrefixOf declName ||
      declName.components.any (· == `Simps) ||
      last.endsWith "_1" || last.endsWith "_2" ||
