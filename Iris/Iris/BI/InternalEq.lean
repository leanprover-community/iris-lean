...
@[rocq_alias fun_extI]
theorem fun_extI {A : Type _} {B : A → Type _} [OFEFun (SI := Nat) B] {f g : (x : A) → B x} :
    (∀ x, f x ≡ g x) ⊢@{PROP} f ≡ g :=
...
theorem discreteFun_equivI_mp {A : Type _} {B : A → Type _} [OFEFun (SI := Nat) B] (f g : (x : A) → B x) :
...
theorem discreteFun_equivI {A : Type _} {B : A → Type _} [OFEFun (SI := Nat) B] (f g : (x : A) → B x) :
    f ≡ g ⊣⊢@{PROP} ∀ x, f x ≡ g x :=
...
theorem internalEq_iff (P Q : PROP) : P ≡ Q ⊢ iprop(P ↔ Q) :=
  letI _ := NonExpansive₂.ne_right (SI := Nat) (BIBase.iff (PROP := PROP)) P
...
theorem affinely_internalEq_wandIff (P Q : PROP) : <affine> (P ≡ Q) ⊢ P ∗-∗ Q :=
  letI _ := NonExpansive₂.ne_right (SI := Nat) (wandIff (PROP := PROP)) P
