...
theorem fixpoint_plain [Sbi PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  ...
  refine ContractiveHom.fixpoint_ind ⟨F, inferInstance⟩ (fun f => ∀ x, Plain (f x)) ?_
      (λ _ => iprop(emp)) inferInstance HΦ ?_
  · intro _ _ H HP x
...
theorem fixpoint_persistent [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  (∀ Φ, (∀ x, Persistent (Φ x)) → (∀ x, Persistent (F Φ x))) → ∀ x, Persistent (fixpoint F x) := by
  intro HΦ
...
theorem fixpoint_absorbing [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  (∀ Φ, (∀ x, Absorbing (Φ x)) → (∀ x, Absorbing (F Φ x))) → ∀ x, Absorbing (fixpoint F x) := by
  intro HΦ
  refine ContractiveHom.fixpoint_ind ⟨F, inferInstance⟩ (fun f => ∀ x, Absorbing (f x)) ?_
      (λ _ => iprop(True)) inferInstance HΦ ?_
  · intro _ _ H HP x
...
theorem fixpoint_persistent_absoring [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  (∀ Φ, (∀ x, Persistent (Φ x)) → (∀ x, Absorbing (Φ x)) → (∀ x, Persistent (F Φ x) ∧ Absorbing (F Φ x))) →
  ...
  · intro _ _ H HP x
    ...
    · refine (equiv_iff.mp (H x)).mpr.trans ?_
      refine (HP x).left.persistent.trans ?_
      exact persistently_mono (equiv_iff.mp (H x)).mp
    · refine (absorbingly_mono (equiv_iff.mp (H x)).mpr).trans ?_
      refine (HP x).right.absorbing.trans ?_
      exact (equiv_iff.mp (H x)).mp
  · refine LimitPreserving.forall _ (fun _ => ?_)
...
theorem fixpoint_persistent_affine [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  (∀ Φ, (∀ x, Persistent (Φ x)) → (∀ x, Affine (Φ x)) → (∀ x, Persistent (F Φ x) ∧ Affine (F Φ x))) →
...
theorem fixpoint_plain_absoring [Sbi PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
...
theorem fixpoint_plain_affine [Sbi PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  (∀ Φ, (∀ x, Plain (Φ x)) → (∀ x, Affine (Φ x)) → (∀ x, Plain (F Φ x) ∧ Affine (F Φ x))) →
