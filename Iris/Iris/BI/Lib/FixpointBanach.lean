...
theorem fixpoint_plain [Sbi PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  ...
  · intro _ _ H HP x
    refine ⟨.trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).plain) ?_⟩
    exact plainly_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
...
theorem fixpoint_persistent [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  ...
  refine ContractiveHom.fixpoint_ind ⟨F, inferInstance⟩ (fun f => ∀ x, Persistent (f x)) ?_
      (fun _ => iprop(emp)) inferInstance HΦ ?_
  · intro _ _ H HP x
    refine ⟨.trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).persistent) ?_⟩
    exact persistently_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
  · refine LimitPreserving.forall _ (fun _ => ?_)
...
theorem fixpoint_absorbing [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  ...
  · intro _ _ H HP x
    refine ⟨.trans (absorbingly_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr) ?_⟩
    exact (HP x).absorbing.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
  · refine LimitPreserving.forall _ (fun _ => ?_)
...
theorem fixpoint_affine [BI PROP] {A : Type _} (F : (A → PROP) → A → PROP) [Contractive F] :
  ...
  refine ContractiveHom.fixpoint_ind ⟨F, inferInstance⟩ (fun f => ∀ x, Affine (f x)) ?_
      (fun _ => iprop(emp)) inferInstance HΦ ?_
  · intro _ _ H HP x
    exact ⟨.trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).affine) .rfl⟩
  · refine LimitPreserving.forall _ (fun _ => ?_)
...
theorem fixpoint_persistent_absorbing [BI PROP] {A : Type _}
  ...
  · intro _ _ H HP x
    refine ⟨⟨?_⟩, ⟨?_⟩⟩
    · refine .trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).left.persistent) ?_
      exact persistently_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
    · refine (absorbingly_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr).trans ?_
      exact (HP x).right.absorbing.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
  · refine LimitPreserving.forall _ (fun _ => ?_)
...
theorem fixpoint_persistent_affine [BI PROP] {A : Type _}
  ...
  · intro _ _ H HP x
    ...
    · refine .trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).left.persistent) ?_
      exact persistently_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
    · exact .trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).right.affine) .rfl
  · refine LimitPreserving.forall _ (fun _ => ?_)
    refine LimitPreserving.and ?_ ?_
    · exact limitPreserving_persistent _ (Φne := ⟨fun _ _ _ h => h _⟩)
    · exact limitPreserving_affine _ (Φne := ⟨fun _ _ _ h => h _⟩)
...
theorem fixpoint_plain_absorbing [Sbi PROP] {A : Type _}
  ...
  · intro _ _ H HP x
    refine ⟨⟨?_⟩, ⟨?_⟩⟩
    · refine .trans (.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr (HP x).left.plain) ?_
      exact plainly_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
    · refine (absorbingly_mono (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mpr).trans ?_
      exact (HP x).right.absorbing.trans (equiv_iff.mp (OFE.Equiv.of_eq (congrFun H x))).mp
  · refine LimitPreserving.forall _ (fun _ => ?_)
