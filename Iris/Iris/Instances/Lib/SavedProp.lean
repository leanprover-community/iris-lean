...
instance saved_anything_fractional (γ : GName) (x : F.ap (IProp GF)) :
  ...
  fractional p q := by
    ...
    exact BIBase.BiEntails.of_eq (congrArg (iOwn _) (DFracAgree.Frac.mk_op (q₁ := p) (q₂ := q)))
...
instance saved_anything_combine_as (γ : GName) (dq1 dq2 : DFrac) (x y : F.ap (IProp GF)) :
  ...
  combine_sep_as := by
    ...
    irewrite [← H] at Hy
    unfold saved_anything_own
    iapply (BIBase.BiEntails.of_eq (congrArg (iOwn _) (DFracAgree.mk_op (d₁ := dq1) (d₂ := dq2) (a := x)))).mpr
    iapply iOwn_op.mpr
