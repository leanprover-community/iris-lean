...
theorem slice_iff {M : Type _ → Type _} [LawfulFiniteMap M SliceName]
  ...
  · imod slice_delete_full E q f P Q γ N HE Hf $$ [$Hslice $Hbox] with ⟨%Pold, HQ, #HeqP, Hbox⟩
    ...
    have hQ' : (iprop(▷ ((Q ↔ Q') ∗ Q)) : IProp GF) ⊢ ▷ Q' := by
      iintro Hc
      inext
      icases Hc with ⟨Hqq, HQ⟩
      ihave Hqq1 := and_elim_l (P := iprop(Q → Q')) (Q := iprop(Q' → Q)) $$ Hqq
      iapply Hqq1 $$ HQ
    icombine Hiff HQ as Hc
    ihave HQ' := hQ' $$ Hc
    ...
    · simp only [BIBase.laterIf]
      inext
      inext
      ihave #Hpp := internalEq_iff P iprop(Q ∗ Pold) $$ HeqP
      iclear HeqP
      icases Hiff with ⟨#Hiff1, #Hiff2⟩
      icases Hpp with ⟨#Hpp1, #Hpp2⟩
      imodintro
      isplit
      · iintro HP
        ihave ⟨HQ, HPold⟩ := Hpp1 $$ HP
        ihave HQ' := Hiff1 $$ HQ
        isplitl [HQ'] <;> iassumption
      · iintro ⟨HQ', HPold⟩
        ihave HQ := Hiff2 $$ HQ'
...
theorem box_internalEq_rewrite {M : Type _ → Type _} [LawfulFiniteMap M SliceName]
    (N : Namespace) (f : M Bool) (P P' : IProp GF) :
    ▷ internalEq P P' ∗ box N f P' ⊢ box N f P := by
  unfold box
  iintro ⟨#Heqpp, %Φ, #Heq, Hbig⟩
  iexists Φ
  isplitl [Heq]
  · icombine Heqpp Heq as Hc
    iapply later_mono _ $$ Hc
    iintro ⟨Hpp, Heq⟩
    iapply internalEq.trans
    isplitl [Hpp] <;> iassumption
  · iexact Hbig
...
theorem slice_split {M : Type _ → Type _} [LawfulFiniteMap M SliceName]
  ...
  intro HE Hf
  have hreassoc : ∀ Pold : IProp GF,
      (iprop((Q1 ∗ Q2) ∗ Pold) : IProp GF) ⊣⊢ iprop(Q2 ∗ (Q1 ∗ Pold)) := by
    intro Pold
    calc (iprop((Q1 ∗ Q2) ∗ Pold) : IProp GF)
      _ ⊣⊢ iprop(Q1 ∗ (Q2 ∗ Pold)) := sep_assoc
      _ ⊣⊢ iprop((Q2 ∗ Pold) ∗ Q1) := sep_comm
      _ ⊣⊢ iprop(Q2 ∗ (Pold ∗ Q1)) := sep_assoc
      _ ⊣⊢ iprop(Q2 ∗ (Q1 ∗ Pold)) := sep_congr_right sep_comm
  cases b
  · iintro ⟨Hslice, Hbox⟩
    ...
    iexists γ1, γ2
    isplit
    · ipureintro; exact Hfresh1
    isplit
    · ipureintro; exact Hfresh2
    isplit
    ...
    isplitl [Hslice2]
    · iexact Hslice2
    have heqPP' : (iprop(internalEq P iprop((Q1 ∗ Q2) ∗ Pold)) : IProp GF) ⊢
        internalEq P iprop(Q2 ∗ (Q1 ∗ Pold)) :=
      (equiv_iff.mp ((internalEq.ne_r P).eqv (equiv_iff.mpr (hreassoc Pold)))).1
    icombine HeqP Hbox as Hc
    have hrepair : (iprop(▷ internalEq P iprop((Q1 ∗ Q2) ∗ Pold) ∗
        box N (insert (insert (delete f γ) γ1 false) γ2 false) iprop(Q2 ∗ (Q1 ∗ Pold)))
          : IProp GF) ⊢
        box N (insert (insert (delete f γ) γ1 false) γ2 false) P :=
      (sep_mono_left (later_mono heqPP')).trans
        (box_internalEq_rewrite N _ P iprop(Q2 ∗ (Q1 ∗ Pold)))
    simp only [BIBase.laterIf]
    iapply laterN_mono q.toNat hrepair $$ Hc
  · iintro ⟨Hslice, Hbox⟩
    imod slice_delete_full E q f P iprop(Q1 ∗ Q2) γ N HE Hf $$ [Hslice Hbox] with
      ⟨%Pold, HQ12, #HeqP, Hbox⟩
    · isplitl [Hslice] <;> iassumption
    icases later_sep.mp $$ HQ12 with ⟨HQ1, HQ2⟩
    imod slice_insert_full E q (delete f γ) Pold Q1 N HE $$ [HQ1 Hbox] with
      ⟨%γ1, %Hfresh1, Hslice1, Hbox⟩
    · isplitl [HQ1] <;> iassumption
    ...
    icombine HeqP Hbox as Hc
    have hrepair : (iprop(▷ internalEq P iprop((Q1 ∗ Q2) ∗ Pold) ∗
        box N (insert (insert (delete f γ) γ1 true) γ2 true) iprop(Q2 ∗ (Q1 ∗ Pold)))
          : IProp GF) ⊢
        box N (insert (insert (delete f γ) γ1 true) γ2 true) P :=
      (sep_mono_left (later_mono heqPP')).trans
        (box_internalEq_rewrite N _ P iprop(Q2 ∗ (Q1 ∗ Pold)))
    simp only [BIBase.laterIf]
    iapply laterN_mono q.toNat hrepair $$ Hc
...
theorem slice_combine {M : Type _ → Type _} [LawfulFiniteMap M SliceName]
  ...
  · iintro ⟨Hslice1, Hslice2, Hbox⟩
    ...
    isplitl [Hslice]
    · iexact Hslice
    have hchain : (iprop(internalEq P iprop(Q1 ∗ Pold1) ∗
        internalEq Pold1 iprop(Q2 ∗ Pold2)) : IProp GF) ⊢
        internalEq P iprop((Q1 ∗ Q2) ∗ Pold2) := by
      letI : NonExpansive (fun B : IProp GF => iprop(Q1 ∗ B)) :=
        NonExpansive₂.ne_right BIBase.sep Q1
      have hcong : (iprop(internalEq Pold1 iprop(Q2 ∗ Pold2)) : IProp GF) ⊢
          internalEq iprop(Q1 ∗ Pold1) iprop(Q1 ∗ (Q2 ∗ Pold2)) :=
        internalEq.of_internalEquiv_ne (PROP := IProp GF) (fun B => iprop(Q1 ∗ B))
      have hassoc : (iprop(internalEq P iprop(Q1 ∗ (Q2 ∗ Pold2))) : IProp GF) ⊢
          internalEq P iprop((Q1 ∗ Q2) ∗ Pold2) :=
        (equiv_iff.mp ((internalEq.ne_r P).eqv (equiv_iff.mpr (hreassoc Pold2).symm))).1
      iintro ⟨H1, H2⟩
      iapply hassoc
      iapply internalEq.trans
      isplitl [H1]
      · iassumption
      · iapply hcong $$ H2
    icombine HeqP1 HeqP2 Hbox as Hc
    have hrepair : (iprop(▷ internalEq P iprop(Q1 ∗ Pold1) ∗
          ▷ internalEq Pold1 iprop(Q2 ∗ Pold2) ∗
        box N (insert (delete (delete f γ1) γ2) γ false) iprop((Q1 ∗ Q2) ∗ Pold2)) : IProp GF) ⊢
        box N (insert (delete (delete f γ1) γ2) γ false) P := by
      iintro ⟨H1, H2, Hbox⟩
      iapply box_internalEq_rewrite N _ P iprop((Q1 ∗ Q2) ∗ Pold2)
      isplitl [H1 H2]
      · icombine H1 H2 as Hc
        iapply later_mono hchain $$ Hc
      · iassumption
    simp only [BIBase.laterIf]
    iapply laterN_mono q.toNat hrepair $$ Hc
  · iintro ⟨Hslice1, Hslice2, Hbox⟩
    ...
    have hrepair : (iprop(▷ internalEq P iprop(Q1 ∗ Pold1) ∗
          ▷ internalEq Pold1 iprop(Q2 ∗ Pold2) ∗
        box N (insert (delete (delete f γ1) γ2) γ true) iprop((Q1 ∗ Q2) ∗ Pold2)) : IProp GF) ⊢
        box N (insert (delete (delete f γ1) γ2) γ true) P := by
      iintro ⟨H1, H2, Hbox⟩
      iapply box_internalEq_rewrite N _ P iprop((Q1 ∗ Q2) ∗ Pold2)
      isplitl [H1 H2]
      · icombine H1 H2 as Hc
