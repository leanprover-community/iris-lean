...
#guard_msgs in #check iprop({{ P }} e {{ x y , RET (x+y) ; Q }})
...
#guard_msgs in #check iprop({{ P }} hl(rec f x := f x) {{ x, RET x; Q }})
/-- info: ⊢ ∀ Φ, P -∗ (▷ ∀ x, Q -∗ Φ x) -∗ WP hl(#1; #2) {{ Φ }} : Prop -/
#guard_msgs in #check {{ P }} hl(#1; #2) {{ x, RET x; Q }}
/-- info: iprop(□ ∀ Φ, P -∗ (▷ ∀ x, Q -∗ Φ x) -∗ WP hl(#1; #2) {{ Φ }} ) : PROP -/
#guard_msgs in #check iprop({{ P }} hl(#1; #2) {{ x, RET x; Q }})
...
#guard_msgs in #check iprop({{ P }} hl(ref(#0)) {{ x, RET x; Q }})
/--
...
-/
#guard_msgs in #check iprop({{ P }} hl(if #1 < #2 then #1 + #1 else #0) {{ x, RET x; Q }})
/-- info: ⊢ ∀ Φ, P -∗ (▷ ∀ v, ⌜v = hl_val(#1)⌝ -∗ Φ v) -∗ WP hl(#1) {{ Φ }} : Prop -/
#guard_msgs in #check {{ P }} hl(#1) {{ v, RET v; ⌜v = hl_val(#1)⌝ }}
