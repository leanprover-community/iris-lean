...
theorem wp_lift_base_step_fupd :
    toVal e₁ = none →
  ...
  imodintro
  isplit
  · ipure_intro; cases s <;> simp only [Stuckness.MaybeReducible, Hred, EctxLanguage.primStep_reducible_of_baseStep_reducible]
  iintro %e₂ %σ₂ %eₜ %Hstep
  iapply H $$ %_ %_ %_
  ipure_intro
  apply EctxLanguage.baseStep_of_primStep_of_baseStep_reducible Hred Hstep

theorem wp_lift_base_step :
    toVal e₁ = none →
    (∀ σ₁ ns obs obs' nt, stateInterp σ₁ ns (obs ++ obs') nt ={E,∅}=∗
      ⌜BaseStep.Reducible (e₁, σ₁)⌝ ∗
      ▷ ∀ e₂ σ₂ eₜ, ⌜(e₁, σ₁) -<obs>->ᵇ (e₂,σ₂,eₜ)⌝ -∗ £ 1 ={∅,E}=∗
...
theorem wp_lift_pure_base_stuck :
    toVal e = none →
    EctxLanguage.SubredexesAreValues e →
    (∀ σ, BaseStep.Stuck (e,σ)) →
    ⊢ WP e @ E ?{{ Φ }} := by
  iintro %toVal_e %sav_e %Hstuck
  iapply wp_lift_base_stuck toVal_e sav_e
  iintro %σ %ns %obs' %nt Hσ
...
theorem wp_lift_atomic_base_step_fupd :
    toVal e₁ = none →
  ...
  imodintro
  isplit
  · ipure_intro; cases s <;> simp [Stuckness.MaybeReducible, EctxLanguage.primStep_reducible_of_baseStep_reducible, *]
  iintro %_ %_ %_ %Hstep
  iapply H
  ipure_intro
  apply EctxLanguage.baseStep_of_primStep_of_baseStep_reducible Hbred Hstep

theorem wp_lift_atomic_base_step :
    toVal e₁ = none →
    (∀ σ₁ ns obs obs' nt, stateInterp σ₁ ns (obs ++ obs') nt ={E}=∗
      ⌜BaseStep.Reducible (e₁, σ₁)⌝ ∗
      ▷ ∀ e₂ σ₂ eₜ, ⌜(e₁, σ₁) -<obs>->ᵇ (e₂, σ₂, eₜ)⌝ -∗ £ 1 ={E}=∗
    ...
    ⊢ WP e₁ @ s; E {{ Φ }} := by
  iintro %toVal_e H
  iapply wp_lift_atomic_step toVal_e
  iintro %σ₁ %ns %obs %obs' %nt Hσ₁
  imod H $$ Hσ₁ with ⟨%Hbred, H⟩
  imodintro
  isplit
...
theorem wp_lift_atomic_base_step_no_fork_fupd :
    toVal e₁ = none →
    (∀ σ₁ ns obs obs' nt, stateInterp σ₁ ns (obs ++ obs') nt ={E1}=∗
      ⌜BaseStep.Reducible (e₁, σ₁)⌝ ∗
      ∀ e₂ σ₂ eₜ, ⌜(e₁, σ₁) -<obs>->ᵇ (e₂, σ₂, eₜ)⌝ -∗ £ 1 ={E1}[E2]▷=∗
    ...
    ⊢ WP e₁ @ s; E1 {{ Φ }} := by
  iintro %toVal_e₁ H
  iapply wp_lift_atomic_base_step_fupd toVal_e₁
  iintro %σ₁ %ns %obs %obs' %nt Hσ₁
...
theorem wp_lift_atomic_base_step_no_fork :
    toVal e₁ = none →
    (∀ σ₁ ns obs obs' nt, stateInterp σ₁ ns (obs ++ obs') nt ={E}=∗
      ⌜BaseStep.Reducible (e₁, σ₁)⌝ ∗
      ▷ ∀ e₂ σ₂ eₜ, ⌜(e₁, σ₁) -<obs>->ᵇ (e₂, σ₂, eₜ)⌝ -∗ £ 1 ={E}=∗
    ...
    ⊢ WP e₁ @ s; E {{ Φ }} := by
  iintro %toVal_e₁ H
  iapply wp_lift_atomic_base_step toVal_e₁
  iintro %σ₁ %ns %obs %obs' %nt Hσ₁
  imod H $$ Hσ₁  with ⟨$, H⟩
  imodintro
  inext
...
theorem wp_lift_pure_det_base_step_no_fork' [Inhabited State] :
  ...
  iintro %toVal_e₁ %Hbred %Hpure _
  refine .trans ?_ <| wp_lift_pure_det_base_step_no_fork (E' := E) toVal_e₁ Hbred Hpure
