  ...
  where
    /--
      Axiomatic interpretation of a state in a language model.
        ...
        - `ns` are the number of prior steps
...
theorem fupd_wp_iff {s : Stuckness}{E}{e : Expr} {Φ : Val → IProp GF} :
  ...
  constructor
  · exact fupd_mask_intro_discard LawfulSet.subset_refl
  · exact fupd_wp
...
theorem wp_credit_access {s : Stuckness} {E : CoPset}{e : Expr}{Φ}{P: IProp GF} :
  ...
  WP e @ s ; E {{ Φ }} := by
    ...
    · simp only [OFE.eq_of_eqv (BI.equiv_iff.mpr lc_split), OFE.eq_of_eqv (BI.equiv_iff.mpr BI.sep_comm)]
      exact .rfl
...
theorem wp_step_fupdN {s : Stuckness} {E₁ E₂ : CoPset} {e : Expr} {P : IProp GF} {Φ : Val → IProp GF} {n : Nat}
  ...
  imod HP
  imod G with toClear; iclear toClear
