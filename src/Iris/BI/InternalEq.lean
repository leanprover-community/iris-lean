...
theorem discrete_eq_mp {A : Type u} [OFE A] {a b : A} [TCOr (DiscreteE a) (DiscreteE b)] :
...
theorem sig_equivI_mp {A : Type u} [OFE A] {P : A → Prop} {x y : Subtype P} :
...
theorem discreteFun_equivI_mp {A : Type u} {B : A → Type u} [OFEFun B] (f g : (x : A) → B x) :
...
theorem discreteFun_equivI {A : Type u} {B : A → Type u} [OFEFun B] (f g : (x : A) → B x) :
    internalEq f g ⊣⊢@{PROP} ∀ x, internalEq (f x) (g x) :=
  ⟨discreteFun_equivI_mp f g, fun_extI⟩

theorem ofeMorO_equivI_mp {A B : Type u} [OFE A] [OFE B] (f g : A -n> B) :
    internalEq f g ⊢@{PROP} ∀ x, internalEq (f x) (g x) := by
...
theorem ofeMorO_equivI_mpr {A B : Type u} [OFE A] [OFE B] (f g : A -n> B) :
...
theorem later_equivI_mp {A : Type u} [OFE A] (x y : A) :
...
theorem later_equivI_mpr {A : Type u} [OFE A] (x y : A) :
...
theorem f_equivI_contractive {A B : Type u} [OFE A] [OFE B] (f : A → B) [hf : Contractive f]
...
instance eq_timeless {A : Type u} [OFE A] (a b : A) [TCOr (DiscreteE a) (DiscreteE b)] :
