...
-- Entails
instance entails_antisymm [BI PROP] : Antisymmetric (α := PROP) (· ⊣⊢ ·) (· ⊢ ·) where
  antisymm := by
    intro _ _ H1 H2
    rw' [equiv_entails]
...
theorem imp_intro' [BI PROP] {P Q R : PROP} : (Q ∧ P ⊢ R) → P ⊢ Q → R := by
  intro H
  apply imp_intro
  rw' [← H]
  apply and_intro
...
theorem imp_elim' [BI PROP] {P Q R : PROP} : (Q ⊢ P → R) → P ∧ Q ⊢ R := by
  intros H
  apply mp (Q := P)
  · rw' [and_elim_r, H]
  · rw' [and_elim_l]
...
instance and_idem [BI PROP] : Idempotent (α := PROP) (· ⊣⊢ ·) (iprop(· ∧ ·)) where
  idem := by
    intro _
    apply antisymm
    · exact and_elim_l
...
instance or_idem [BI PROP] : Idempotent (α := PROP) (· ⊣⊢ ·) (iprop(· ∨ ·)) where
  idem := by
    intro _
    apply antisymm
    · apply or_elim
...
instance and_comm [BI PROP] : Commutative (α := PROP) (· ⊣⊢ ·) (iprop(· ∧ ·)) where
  comm := by
    intros
    apply antisymm
    case left =>
...
instance and_leftId [BI PROP] : LeftId (α := PROP) (· ⊣⊢ ·) iprop(True) (iprop(· ∧ ·)) where
  left_id := by
    intros
    apply antisymm
    case left =>
      exact and_elim_r
    case right =>
      apply and_intro ?_ refl
      apply pure_intro
      simp

instance and_rightId [BI PROP] : RightId (α := PROP) (· ⊣⊢ ·) iprop(True) (iprop(· ∧ ·)) where
  right_id := by
    intros
    apply antisymm
    case left =>
      exact and_elim_l
    case right =>
      apply and_intro refl ?_
      apply pure_intro
      simp

instance and_assoc [BI PROP] : Associative (α := PROP) (· ⊣⊢ ·) (iprop(· ∧ ·)) where
  assoc := by
    intro _ _ _
    apply antisymm
    <;> apply and_intro
...
instance sep_comm [BI PROP] : Commutative (α := PROP) (· ⊣⊢ ·) (iprop(· ∗ ·)) where
  comm := by
    intros
    apply antisymm
    <;> exact sep_symm

instance sep_assoc [BI PROP] : Associative (α := PROP) (· ⊣⊢ ·) (iprop(· ∗ ·)) where
  assoc := by
    intros P Q R
    apply antisymm
    case left =>
...
theorem true_sep_2 [BI PROP] {P : PROP} : P ⊢ True ∗ P := by
  rw' [emp_sep_2]
  apply sep_mono ?_ refl
  apply pure_intro
  simp

theorem wand_intro' [BI PROP] {P Q R : PROP} : (Q ∗ P ⊢ R) → P ⊢ Q -∗ R := by
  rw' [(comm : Q ∗ P ⊣⊢ _)]
  exact wand_intro
...
theorem wand_elim' [BI PROP] {P Q R : PROP} : (Q ⊢ P -∗ R) → P ∗ Q ⊢ R := by
  intro H
  rw' [H, wand_elim_r]
...
theorem wand_rfl [BI PROP] {P : PROP} : ⊢ P ∗-∗ P := by
  apply and_intro
  <;> apply wand_intro'
  <;> rw' [(right_id : P ∗ emp ⊣⊢ _)]
...
theorem equiv_wandIff [BI PROP] {P Q : PROP} : (P ⊣⊢ Q) → ⊢ P ∗-∗ Q := by
  intro H
  rw' [H]
  exact wand_rfl

theorem wandIff_equiv [BI PROP] {P Q : PROP} : (⊢ P ∗-∗ Q) → (P ⊣⊢ Q) := by
  intro HPQ
  apply antisymm
  <;> apply wand_entails
  <;> rw' [HPQ]
...
theorem pure_imp_2 {φ1 φ2 : Prop} [BI PROP] : ⌜φ1 → φ2⌝ ⊢ (⌜φ1⌝ → ⌜φ2⌝ : PROP) := by
  apply imp_intro'
  rw' [← pure_and]
  apply pure_mono
  simp_all

theorem pure_forall_2 {φ : α → Prop} [BI PROP] : ⌜∀ x, φ x⌝ ⊢ ∀ x, (⌜φ x⌝ : PROP) := by
  apply forall_intro
  intro x
  apply pure_mono
  simp_all
...
-- Persistent
theorem absorbingly_persistently [BI PROP] {P : PROP} : <absorb> <pers> P ⊣⊢ <pers> P := by
  apply antisymm
  case left =>
    simp only [absorbingly]
    rw' [
...
theorem persistently_or [BI PROP] {P Q : PROP} : <pers> (P ∨ Q) ⊣⊢ <pers> P ∨ <pers> Q := by
  rw' [!or_eq_ite, persistently_exists]
  apply antisymm
  <;> apply exists_elim
  <;> intro a
...
theorem absorbingly_of_persistently [BI PROP] {P : PROP} : <pers> P ⊢ <absorb> P := by
  rw' [
...
theorem persistently_and_imp_sep [BI PROP] {P Q : PROP} : <pers> P ∧ Q ⊢ <pers> P ∗ Q := by
  conv =>
    lhs
...
theorem persistently_and_persistently_sep [BI PROP] {P Q : PROP} : <pers> P ∧ <pers> Q ⊣⊢ <pers> P ∗ <pers> Q := by
  apply antisymm
  case left =>
    exact persistently_and_imp_sep
  case right =>
    apply and_intro
...
theorem persistently_sep_2 [BI PROP] {P Q : PROP} : <pers> P ∗ <pers> Q ⊢ <pers> (P ∗ Q) := by
  rw' [← persistently_and_sep, persistently_and, ← persistently_and_persistently_sep]
...
theorem intuitionistically_true [BI PROP] : □ True ⊣⊢ (emp : PROP) := by
...
theorem persistently_of_intuitionistically [BI PROP] {P : PROP} : □ P ⊢ <pers> P := by
  rw' [affinely_elim]

theorem intuitionistically_persistently [BI PROP] {P : PROP} : □ <pers> P ⊣⊢ □ P := by
  simp only [intuitionistically]
  rw' [persistently_idem]

theorem intuitionistically_of_intuitionistic [BI PROP] {P : PROP} [Affine P] [Persistent P] : □ P ⊣⊢ P := by
  apply antisymm
...
theorem affinely_of_intuitionistically [BI PROP] {P : PROP} : □ P ⊢ <affine> P := by
  simp only [intuitionistically, affinely]
  apply and_intro
  · exact and_elim_l
  · exact persistently_and_emp_elim

theorem intuitionistically_affinely [BI PROP] {P : PROP} : □ <affine> P ⊣⊢ □ P := by
  simp only [intuitionistically]
...
theorem intuitionistically_iff_persistently [BIAffine PROP] {P : PROP} : □ P ⊣⊢ <pers> P := by
...
theorem intuitionisticallyIf_of_intuitionistically (p : Bool) [BI PROP] {P : PROP} : □ P ⊢ □?p P := by
  cases p
...
theorem absorbingly_intuitionistically [BI PROP] {P : PROP} :
  <absorb> □ P ⊣⊢ <pers> P
:= by
  apply antisymm
  case left =>
    rw' [
      persistently_of_intuitionistically,
...
theorem absorbingly_affinely_intro_of_persistent [BI PROP] {P : PROP} [Persistent P] : P ⊢ <absorb> <affine> P := by
  rw' [
    persistent,
