...
theorem HγLookup : FF1[γ] = COFE.OFunctor.constOF (Agree (LeibnizO String)) := rfl
...
theorem MyR_always_invalid (S₁ S₂ : String) (Hne : S₁ ≠ S₂) (n : Nat) : ¬✓{n} MyR S₁ • MyR S₂ := by
  simp [CMRA.op, DiscreteFunO.discrete_fun_op, CMRA.ValidN, DiscreteFunO.discrete_fun_validN]
  exists γ, ⟨0⟩
