...
/- Paramaterize by a type of UFractions.
This example also shows how to deal with this kind of polymorphism, which can be tricky. -/
...
/- Allow the types of GF and F, and the UFraction instance for F, to be inferred by γ alone.
Doing it this way allows us to define a notation for the points-to that does not
Curiously, I don't need to instanstiate this; having it in the context is enough for typeclass
inference to make this shortcut.
I sort of doubt that we would want to do this globally for ElemG, but I'm not sure about that. -/
set_option synthInstance.checkSynthOrder false in
class abbrev F1PointsTo (γ : GName) (GF : outParam _) (F : outParam (Type _)) [UFraction F] :=
  ElemG GF (F1 (F := F))

/- Define notation for the heap -/
def PointsTo (γ : GName) [F1PointsTo γ GF F] (k : Nat) (v : String) :=
  iOwn (GF := GF) (F := F1 (F := F)) γ (Frag k (own one) (toAgree ⟨v⟩))

notation k:50 " ↦[" γ:50 "] " v:50 => PointsTo γ k v

/- In this example, we assume that the setup is complete and a GName has already been allocated. -/
example {γ : GName} : 5 ↦[γ] "A" ∗ 5 ↦[γ] "B" ⊢ False := by
  ...
  iintro %H
  have _ := dist_inj <| toAgree_op_validN_iff_dist.mp <| (frag_op_validN_iff.mp H).2
  grind
