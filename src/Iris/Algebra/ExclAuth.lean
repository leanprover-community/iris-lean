...
instance auth_discrete {a : A} [DiscreteE a] : DiscreteE (●E a : ExclAuthR (F := F)) :=
  Auth.auth_discrete (some_is_discrete inferInstance) none_is_discrete

@[rocq_alias excl_auth_frag_discrete]
instance frag_discrete {a : A} [DiscreteE a] : DiscreteE (◯E a : ExclAuthR (F := F)) :=
  Auth.frag_discrete (some_is_discrete inferInstance)

@[rocq_alias excl_auth_validN]
theorem validN {n : Nat} {a : A} : ✓{n} (●E a : ExclAuthR (F := F)) • ◯E a :=
  Auth.both_validN.mpr ⟨.rfl, trivial⟩

@[rocq_alias excl_auth_valid]
theorem valid {a : A} : ✓ (●E a : ExclAuthR (F := F)) • ◯E a :=
  Auth.auth_both_valid_2 trivial .rfl

@[rocq_alias excl_auth_agreeN]
theorem agreeN {n : Nat} {a b : A} (h : ✓{n} (●E a : ExclAuthR (F := F)) • ◯E b) :
    a ≡{n}≡ b := dist_of_inc_exclusive (Auth.both_validN.mp h).1 trivial |>.symm

@[rocq_alias excl_auth_agree]
theorem agree {a b : A} (h : ✓ (●E a : ExclAuthR (F := F)) • ◯E b) : a ≡ b :=
  equiv_dist.mpr fun _ => agreeN (Valid.validN h)

@[rocq_alias excl_auth_agree_L]
theorem agree_L [Leibniz A] {a b : A} (h : ✓ (●E a : ExclAuthR (F := F)) • ◯E b) :
    a = b :=
  eq_of_eqv (agree h)

@[rocq_alias excl_auth_auth_op_validN]
theorem auth_op_validN {n : Nat} {a b : A} : (✓{n} ((●E a : ExclAuthR (F := F)) • ●E b)) ↔ False :=
  Auth.auth_op_validN

@[rocq_alias excl_auth_auth_op_valid]
theorem auth_op_valid {a b : A} : (✓ (●E a : ExclAuthR (F := F)) • ●E b) ↔ False :=
  Auth.auth_op_valid

@[rocq_alias excl_auth_frag_op_validN]
theorem frag_op_validN  {a b : A} : (✓{n} (◯E a : ExclAuthR (F := F)) • ◯E b) ↔ False := by
...
theorem frag_op_valid {a b : A} : (✓ (◯E a : ExclAuthR (F := F)) • ◯E b) ↔ False := by
...
theorem update {a b a' : A} : ((●E a : ExclAuthR (F := F)) • ◯E b) ~~> ((●E a') • ◯E a') :=
