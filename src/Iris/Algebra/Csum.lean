...
instance [OFE α] [OFE β] : OFE (Csum α β) where
  ...
  equiv_dist {x y} := by
    ...
    · cases x <;> cases y <;> first | exact Equiv.dist h | trivial
    · cases x <;> cases y <;> first | exact equiv_dist.mpr h | exact (h 0).elim | trivial
  dist_lt {n x y m} hn hlt := by
    cases x <;> cases y <;> first | exact Dist.lt hn hlt | exact hn.elim | trivial
...
@[rocq_alias Cinl_inj]
theorem Csum.inl_inj [OFE α] [OFE β] {a a' : α}
    (h : (inl (β := β) a) ≡ inl a') : a ≡ a' := h
...
@[rocq_alias Cinr_inj]
theorem Csum.inr_inj [OFE α] [OFE β] {b b' : β}
    (h : (inr (α := α) b) ≡ inr b') : b ≡ b' := h
...
@[simp] def Csum.getInlD (x : Csum α β) (d : α) : α :=
  match x with | inl a => a | _ => d

@[simp] def Csum.getInrD (x : Csum α β) (d : β) : β :=
...
instance [CMRA α] [CMRA β] : CMRA (Csum α β) where
  ...
  op_ne {x} := ⟨fun {n y₁ y₂} hy => by cases x <;> cases y₁ <;> cases y₂ <;>
    first | exact Dist.op_r hy | exact hy | trivial⟩
...
theorem csum_map_ext [OFE α] [OFE α'] [OFE β] [OFE β'] (f f' : α → α') (g g' : β → β')
  ...
  cases x with
  | inl a => simp [Csum.map]; exact hf _
  | inr b => simp [Csum.map]; exact hg _
  | invalid => trivial
...
theorem csum_map_ne [OFE α] [OFE α'] [OFE β] [OFE β'] {n}
  ...
  cases x with
  | inl => cases y with | inl => simp [Csum.map]; exact hf hxy | _ => exact hxy
  | inr => cases y with | inr => simp [Csum.map]; exact hg hxy | _ => exact hxy
...
theorem csumO_map_ne [OFE α] [OFE α'] [OFE β] [OFE β'] :
  ...
  ne _ _ _ hf _ _ hg x := by
    cases x with
    | inl => simp [csumO_map, Csum.map]; exact hf _
    | inr => simp [csumO_map, Csum.map]; exact hg _
    | invalid => trivial
...
instance {Fa Fb} [RFunctor Fa] [RFunctor Fb] : RFunctor (CsumOF Fa Fb) where
  ...
  map_ne.ne _ _ _ hf _ _ hg x := by
    cases x <;> simp [csumC_map, Csum.map] <;> exact RFunctor.map_ne.ne hf hg _
  map_id x := by cases x <;> simp [csumC_map, Csum.map] <;> exact RFunctor.map_id _
  map_comp f g f' g' x := by
    cases x <;> simp [csumC_map, Csum.map] <;> exact RFunctor.map_comp f g f' g' _
