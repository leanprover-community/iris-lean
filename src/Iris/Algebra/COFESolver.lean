...
local instance {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] : Inhabited ((D -n> Fix F) × (Fix F -n> D)) where
  default := ⟨⟨Function.const _ default, inferInstance⟩, ⟨Function.const _ default, inferInstance⟩⟩

def Fix.uniqueBody {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D))
  : (D -n> Fix F) × (Fix F -n> D) → (D -n> Fix F) × (Fix F -n> D) :=
    fun g => ⟨(fold.comp (map g.2 g.1)).comp ı.hom, (ı.inv.comp (map g.1 g.2)).comp unfold⟩

local instance Fix.uniqueBody_contractive {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) : Contractive (uniqueBody ı) where
  distLater_dist {n x y} H := by
    apply dist_prod_ext
    · intro a; dsimp
      have H' : DistLater n (x.snd, x.fst) (y.snd, y.fst) := by
        cases n with
        | zero => exact distLater_zero
        | succ _ => exact distLater_succ.mpr ⟨(distLater_succ.mp H).2, (distLater_succ.mp H).1⟩
      exact NonExpansive.ne ((OFunctorContractive.map_contractive (F := F)).1 H' _)
    · intro a; dsimp
      exact NonExpansive.ne ((OFunctorContractive.map_contractive (F := F)).1 H _)

def Fix.uniqueBodyFix {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) :
  (D -n> Fix F) × (Fix F -n> D) -c> (D -n> Fix F) × (Fix F -n> D) := ⟨uniqueBody ı, inferInstance⟩

abbrev Fix.uniqueMediators {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) : (D -n> Fix F) × (Fix F -n> D) :=
  ContractiveHom.fixpoint (uniqueBodyFix ı)

theorem Fix.uniqueMediators_hom_inv {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) :
    let g := (uniqueMediators ı)
    g.fst.comp g.snd ≡ Hom.id := by
  intro g
  refine distLater_soundness (fun n IH x => ?_)
  have hequiv : g ≡ uniqueBodyFix ı g := fixpoint_unfold _
  have IH' : DistLater n (g.fst.comp g.snd, g.fst.comp g.snd) (Hom.id, Hom.id) := by
    cases n with
    | zero => exact distLater_zero
    | succ _ => exact distLater_succ.mpr ⟨(distLater_succ.mp IH), (distLater_succ.mp IH)⟩
  refine (g.fst.ne.eqv (equiv_snd hequiv x)).dist.trans ?_
  refine ((equiv_fst hequiv) (((uniqueBodyFix ı).f g).snd.f _)).dist.trans ?_
  simp only [uniqueBodyFix, uniqueBody]
  refine (fold.ne.eqv ((map g.snd g.fst).ne.eqv ı.hom_inv)).dist.trans ?_
  refine (fold.ne.eqv ((map_comp g.fst g.snd g.snd g.fst) _).symm).dist.trans ?_
  refine (fold.ne.ne (OFunctorContractive.map_contractive.1 IH' _)).trans ?_
  refine (fold.ne.eqv ((map_id) _)).dist.trans ?_
  exact (Fix.fold_unfold _).dist

theorem Fix.uniqueMediators_inv_hom {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) :
    let g := (uniqueMediators ı)
    g.snd.comp g.fst ≡ Hom.id := by
  intro g
  refine distLater_soundness (fun n IH x => ?_)
  have hequiv : g ≡ uniqueBodyFix ı g := fixpoint_unfold _
  have IH' : DistLater n (g.snd.comp g.fst, g.snd.comp g.fst) (Hom.id, Hom.id) := by
    cases n with
    | zero => exact distLater_zero
    | succ _ => exact distLater_succ.mpr ⟨(distLater_succ.mp IH), (distLater_succ.mp IH)⟩
  refine (g.snd.ne.eqv (equiv_fst hequiv x)).dist.trans ?_
  refine ((OFE.equiv_snd hequiv) (((uniqueBodyFix ı).f g).fst.f x)).dist.trans ?_
  simp only [uniqueBodyFix, uniqueBody]
  refine (ı.inv.ne.eqv ((map g.fst g.snd).ne.eqv (unfold_fold _))).dist.trans ?_
  refine (ı.inv.ne.eqv ((map_comp g.snd g.fst g.fst g.snd) _).symm).dist.trans ?_
  refine (ı.inv.ne.ne (OFunctorContractive.map_contractive.1 IH' _)).trans ?_
  refine (ı.inv.ne.eqv ((map_id) _)).dist.trans ?_
  exact ı.inv_hom.dist

def Fix.unique {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) : Iso D (Fix F) where
  hom := (uniqueMediators ı).1
  inv := (uniqueMediators ı).2
  hom_inv := uniqueMediators_hom_inv ı _
  inv_hom := uniqueMediators_inv_hom ı _

theorem Fix.unique_hom {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) :
  (unique ı).hom ≡ (fold.comp (map (unique ı).inv (unique ı).hom)).comp ı.hom := by
  simp only [unique, uniqueMediators]
  have hequiv : (uniqueBodyFix ı).fixpoint ≡ uniqueBodyFix ı (uniqueBodyFix ı).fixpoint := fixpoint_unfold _
  exact (equiv_fst hequiv).trans .rfl

theorem Fix.unique_inv {D : Type u} [OFE D] [IsCOFE D] [Inhabited D] (ı : Iso D (F D D)) :
  (unique ı).inv ≡ (ı.inv.comp (map (unique ı).hom (unique ı).inv)).comp unfold := by
  simp only [unique, uniqueMediators]
  have hequiv : (uniqueBodyFix ı).fixpoint ≡ uniqueBodyFix ı (uniqueBodyFix ı).fixpoint := fixpoint_unfold _
  exact (equiv_snd hequiv).trans .rfl
