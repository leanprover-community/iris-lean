...
abbrev equiv (x y : View F R) : Prop := x.auth ≡ y.auth ∧ x.frag ≡ y.frag
abbrev dist (n : Nat) (x y : View F R) : Prop := x.auth ≡{n}≡ y.auth ∧ x.frag ≡{n}≡ y.frag
...
instance mk.ne : OFE.NonExpansive₂ (mk : _ → _ → View F R) := ⟨fun _ _ _ Ha _ _ Hb => ⟨Ha, Hb⟩⟩
instance auth.ne : OFE.NonExpansive (auth : View F R → _) := ⟨fun _ _ _ H => H.1⟩
instance frag.ne : OFE.NonExpansive (frag : View F R → _) := ⟨fun _ _ _ H => H.2⟩

theorem is_discrete {ag : Option ((DFrac F) × Agree A)} (Ha : OFE.DiscreteE ag) (Hb : OFE.DiscreteE b) :
  OFE.DiscreteE (α := View F R) (mk ag b) := ⟨fun H => ⟨Ha.discrete H.1, Hb.discrete H.2⟩⟩
...
theorem auth_inj_ag [UCMRA B] {q1 q2 : DFrac F} {a1 a2 : A} {n} (H : (●V{q1} a1 : View F R) ≡{n}≡ ●V{q2} a2) :
    a1 ≡{n}≡ a2 := toAgree.inj H.1.2

theorem frag_inj [UCMRA B] {b1 b2 : B} {n} (H : (◯V b1 : View F R) ≡{n}≡ ◯V b2) :
    b1 ≡{n}≡ b2 := H.2

theorem auth_is_discrete [UFraction F] [UCMRA B] {dq a} (Ha : OFE.DiscreteE a) (He : OFE.DiscreteE (UCMRA.unit : B)) :
...
theorem frag_is_discrete [UCMRA B] {b : B} (Hb : OFE.DiscreteE b) : (OFE.DiscreteE (◯V b : View F R)) :=
...
theorem IsViewRel.of_agree_dst_iff (Hb : b' ≡{n}≡ b) :
...
instance : CMRA (View F R) where
  ...
  validN_ne {n x1 x2} := by
    ...
    simp_all
    · exact fun x H => ⟨x, IsViewRel.mono H .rfl Hr.symm.to_incN n.le_refl⟩
    intro Hq a Hag HR
    refine ⟨CMRA.discrete_valid <| DFrac_CMRA.validN_ne Hl.1 Hq, ?_⟩
    refine ⟨a, ?_⟩
    refine ⟨Hl.2.symm.trans Hag, ?_⟩
    refine IsViewRel.mono HR .rfl ?_ n.le_refl
  ...
  validN_op_left {n x y} := by
    ...
    · refine fun Hq a H Hr => ⟨CMRA.validN_op_left Hq, ⟨a, ?_, ?_⟩⟩
      ...
      · exact IsViewRel.mono Hr .rfl (CMRA.incN_op_left n b1 b2) n.le_refl
  assoc := OFE.NonExpansive₂.eqv CMRA.assoc CMRA.assoc
  comm := OFE.NonExpansive₂.eqv CMRA.comm CMRA.comm
  pcore_op_left {x _} := by
    simp only [Pcore, Option.some.injEq]
    exact fun H => H ▸ OFE.NonExpansive₂.eqv (CMRA.core_op x.auth) (CMRA.core_op x.frag)
  pcore_idem {_ cx} := by
...
instance [OFE.Discrete A] [CMRA.Discrete B] [IsViewRelDiscrete R] : CMRA.Discrete (View F R) where
  discrete_valid := by
    ...
    · rintro ⟨a, H⟩ _
      exact ⟨a, IsViewRelDiscrete.discrete _ _ _ H⟩
...
theorem auth_validN_iff : ✓{n} (●V{dq} a : View F R) ↔ ✓{n}dq ∧ R n a UCMRA.unit :=
  and_congr_right fun _ => IsViewRel.of_agree_dst_iff .rfl
...
theorem auth_op_auth_validN_iff :
  ...
  · let Ha' : a1 ≡{n}≡ a2 := dst_of_validN_auth H
    ...
    refine ⟨Hq, Ha', IsViewRel.mono HR ?_ CMRA.incN_unit n.le_refl⟩
  ...
  · simp [CMRA.op, CMRA.ValidN, ValidN, optionOp, Prod.op]
    ...
    · refine IsViewRel.mono H.2.2 .rfl ?_ n.le_refl
...
theorem auth_op_frag_validN_iff : ✓{n} ((●V{dq} a : View F R) • ◯V b) ↔ ✓dq ∧ R n a b :=
  and_congr_right (fun _ => IsViewRel.of_agree_dst_iff <| CMRA.unit_left_id_dist b)
...
theorem auth_valid_iff : ✓ (●V{dq} a : View F R) ↔ ✓dq ∧ ∀ n, R n a UCMRA.unit :=
  and_congr_right (fun _=> forall_congr' fun _ => IsViewRel.of_agree_dst_iff .rfl)
...
theorem auth_op_auth_valid_iff : ✓ ((●V{dq1} a1 : View F R) • ●V{dq2} a2) ↔ ✓(dq1 • dq2) ∧ a1 ≡ a2 ∧ ∀ n, R n a1 UCMRA.unit := by
  ...
  · simp [valid, CMRA.op, op, optionOp, CMRA.ValidN, ValidN] at H
    ...
    · rcases (H n) with ⟨_, _, Hl, H⟩
      apply IsViewRel.mono H ?_ CMRA.incN_unit n.le_refl
      apply toAgree.inj
      apply Hl.symm.trans
...
theorem auth_op_frag_valid_iff : ✓ ((●V{dq} a : View F R) • ◯V b) ↔ ✓dq ∧ ∀ n, R n a b :=
  and_congr_right (fun _ => forall_congr' fun _ => IsViewRel.of_agree_dst_iff <| CMRA.unit_left_id_dist b)
...
theorem auth_op_frag_update {Pab : A → B → Prop}
  ...
  · intro H
    simp [CMRA.op, CMRA.ValidN, optionOp, ValidN] at H
    obtain ⟨_, a0, He', Hrel'⟩ := H
    have He := toAgree.inj He'; clear He'
    have Hrel : R n a (b • bf) := by
      apply IsViewRel.mono Hrel' He.symm _ n.le_refl
...
theorem frag_update (Hupd : ∀ a n bf, R n a (b • bf) → R n a (b' • bf)) :
  ...
  · simp_all [CMRA.op, optionOp]
    ...
    exists a
    apply Hupd _ _ _ HR
  · simp_all [CMRA.op, optionOp, CMRA.ValidN, ValidN]
    intro Hq a He Hr
    exists a
...
theorem auth_alloc (Hup : ∀ n bf, R n a bf → R n a (b • bf)) :
  ...
  · simp [CMRA.op, optionOp, CMRA.ValidN, ValidN]
    ...
    have HR' := IsViewRel.mono HR He.symm (CMRA.incN_op_right n UCMRA.unit bf) n.le_refl
    apply IsViewRel.mono (Hup n bf HR') He ?_ n.le_refl
    apply Iris.OFE.Dist.to_incN
    refine CMRA.comm.dist.trans (.trans ?_ CMRA.comm.dist)
    refine CMRA.op_ne.ne ?_
