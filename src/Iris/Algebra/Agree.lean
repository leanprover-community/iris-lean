...
-- For Agree values, 0-equivalence holds whenever there's a bijection between elements
-- that preserves 0-equivalence
theorem agree_dist_zero {α : Type u} [OFE α] (x y : Agree α) :
    x ≡{0}≡ y ↔ (∀ a ∈ x.car, ∃ b ∈ y.car, a ≡{0}≡ b) ∧ (∀ b ∈ y.car, ∃ a ∈ x.car, a ≡{0}≡ b) := by
  rfl
...
/-- The agreement construction as an OFunctorPre -/
abbrev agreeOF : OFunctorPre := fun _ β _ _ => Agree β


instance : OFunctor agreeOF where
  cofe := inferInstance
  map _ g := ⟨fun x => ⟨x.car.map g.f, by
    cases h : x.car
    · exact absurd h x.not_nil
    · simp⟩, by
    constructor
    intro n x₁ x₂ h
    constructor
    · intro a ha
      ...
      exact ⟨g.f b', by simp only [List.mem_map]; exact ⟨b', hb', rfl⟩, g.ne.ne hab'⟩
    · intro b hb
      rw [List.mem_map] at hb
      obtain ⟨b', hb', rfl⟩ := hb
      obtain ⟨a', ha', hab'⟩ := h.2 b' hb'
      exact ⟨g.f a', by simp only [List.mem_map]; exact ⟨a', ha', rfl⟩, g.ne.ne hab'⟩⟩
  map_ne := by
    intro α₁ α₂ β₁ β₂ _ _ _ _
    constructor
    intro n x₁ x₂ hx y₁ y₂ hy z
    simp only []
    constructor
    · intro a ha
      rw [List.mem_map] at ha
      obtain ⟨a', ha', rfl⟩ := ha
      exact ⟨y₂.f a', by simp only [List.mem_map]; exact ⟨a', ha', rfl⟩, hy a'⟩
    · intro b hb
      rw [List.mem_map] at hb
      obtain ⟨b', hb', rfl⟩ := hb
      exact ⟨y₁.f b', by simp only [List.mem_map]; exact ⟨b', hb', rfl⟩, (hy b')⟩
  map_id x := by
    intro n
    simp only [Agree.dist]
    have : x.car.map OFE.Hom.id.f = x.car := by
      rw [show OFE.Hom.id.f = id from rfl, List.map_id]
    rw [this]
    exact ⟨fun a ha => ⟨a, ha, .rfl⟩, fun a ha => ⟨a, ha, .rfl⟩⟩
  map_comp f g f' g' x := by
    intro n
    simp only [Agree.dist]
    -- Need to show that map (f.comp g) (g'.comp f') x ≡ map g g' (map f f' x)
    -- Left side: x.car.map (g'.comp f').f = x.car.map (λ a => g'.f (f'.f a))
    -- Right side: (x.car.map f'.f).map g'.f = x.car.map (λ a => g'.f (f'.f a))
    have eq : x.car.map (g'.comp f').f = (x.car.map f'.f).map g'.f := by
      rw [List.map_map]
      rfl
    rw [eq]
    exact ⟨fun a ha => ⟨a, ha, .rfl⟩, fun a ha => ⟨a, ha, .rfl⟩⟩

instance : RFunctor agreeOF where
  cmra {α β} _ _ := inferInstanceAs (CMRA (Agree β))
  map f g := {
    toHom := OFunctor.map f g
    validN := by
      intro n x hx
      -- Need to show that OFunctor.map f g x is n-valid
      -- The mapped value is ⟨x.car.map g.f, _⟩
      -- The result of OFunctor.map f g x is { car := List.map g.f x.car, not_nil := _ }
      change Agree.validN n { car := List.map g.f x.car, not_nil := _ }
      rw [Agree.validN_iff]
      -- We need to show: ∀ a ∈ x.car.map g.f, ∀ b ∈ x.car.map g.f, a ≡{n}≡ b
      intro a ha b hb
      -- Since a and b are in the mapped list, they come from some elements
      rcases List.mem_map.mp ha with ⟨a', ha', rfl⟩
      rcases List.mem_map.mp hb with ⟨b', hb', rfl⟩
      -- Since x is valid, we know a' ≡{n}≡ b'
      have hab' : a' ≡{n}≡ b' := by
        exact Agree.validN_iff.mp hx a' ha' b' hb'
      -- Since g is non-expansive, g.f preserves n-equivalence
      exact g.ne.ne hab'
    pcore := by
      intro x
      -- For Agree, pcore x = some x always
      rfl
    op := by
      intro x y
      -- Need to show: OFunctor.map f g (x • y) ≡{n}≡ (OFunctor.map f g x) • (OFunctor.map f g y)
      intro n
      simp only [Agree.dist]
      -- We need to show the mapped operations are n-equivalent
      -- (x • y).car = x.car ++ y.car, so mapping gives (x.car ++ y.car).map g.f
      -- The RHS is (map g x) • (map g y), which has car = (x.car.map g.f) ++ (y.car.map g.f)
      -- By List.map_append, these are equal
      have eq : (x.car ++ y.car).map g.f = (x.car.map g.f) ++ (y.car.map g.f) := List.map_append
      -- Since the car components are equal, the Agree values are n-equivalent
      constructor
      · intro a ha
        -- (x • y).car = x.car ++ y.car
        -- So ha : a ∈ (x.car ++ y.car).map g.f
        -- ha : a ∈ (OFunctor.map f g (x • y)).car = (x • y).car.map g.f
        -- Since (x • y).car = x.car ++ y.car, we have ha : a ∈ (x.car ++ y.car).map g.f
        -- Unfold the operation to see that (x • y).car = x.car ++ y.car
        change a ∈ (x.car ++ y.car).map g.f at ha
        rw [eq] at ha
        -- Now ha : a ∈ (x.car.map g.f) ++ (y.car.map g.f)
        -- This is exactly the car of (OFunctor.map f g x) • (OFunctor.map f g y)
        exact ⟨a, ha, .rfl⟩
      · intro a ha
        -- ((OFunctor.map f g x) • (OFunctor.map f g y)).car = (x.car.map g.f) ++ (y.car.map g.f)
        -- So ha : a ∈ (x.car.map g.f) ++ (y.car.map g.f)
        -- ha : a ∈ ((OFunctor.map f g x) • (OFunctor.map f g y)).car
        -- Since •.car = ++ for Agree, ha : a ∈ (x.car.map g.f) ++ (y.car.map g.f)
        -- We need to show ∃ a_1, a_1 ∈ List.map g.f (x • y).car ∧ a_1 ≡{n}≡ a
        -- Since (x • y).car = x.car ++ y.car, we have
        -- List.map g.f (x • y).car = List.map g.f (x.car ++ y.car)
        -- By eq, this equals (x.car.map g.f) ++ (y.car.map g.f)
        -- And ha tells us a is in this list
        refine ⟨a, ?_, .rfl⟩
        -- ha : a ∈ (OFunctor.map f g x • OFunctor.map f g y).car
        -- By definition of op, this means a ∈ (x.car.map g.f ++ y.car.map g.f)
        change a ∈ (x.car.map g.f ++ y.car.map g.f) at ha
        rw [←eq] at ha
        -- Now ha : a ∈ (x.car ++ y.car).map g.f = (x • y).car.map g.f
        exact ha
  }
