...
instance : IsViewRel (authViewRel (A := A)) where
  ...
  rel_unit n := by
    exists unit
    constructor
    · exact incN_refl unit
    · exact unit_valid.validN

theorem auth_view_rel_unit {n : Nat} {a : A} : authViewRel n a unit ↔ ✓{n} a := by
  constructor
  · intro ⟨_, Hv⟩; exact Hv
  · intro Hv; exact ⟨incN_unit, Hv⟩

theorem auth_view_rel_exists {n : Nat} {b : A} : (∃ a, authViewRel n a b) ↔ ✓{n} b := by
  constructor
...
theorem auth_both_core_frac q a b :
...
theorem frag_validN {n : Nat} {b : A} :
    (✓{n} (◯ b : Auth F A)) ↔ (✓{n} b) := by
  rw [View.frag_validN_iff, authViewRel.auth_view_rel_exists]
...
theorem auth_dfrac_valid {dq : DFrac F} {a : A} :
  ...
  rw [valid_iff_validN]
  exact forall_congr' fun _ => authViewRel.auth_view_rel_unit
...
theorem auth_update {a b a' b' : A}
  ...
  have Ha_eq : a ≡{n}≡ b •? some (bf • c) := by
    simp only [CMRA.op?]
    calc a ≡{n}≡ (b • bf) • c := Hinc
         _ ≡{n}≡ b • (bf • c) := assoc.symm.dist
  have ⟨Hv', Ha'_eq⟩ := Hup n (some (bf • c)) Hv Ha_eq
  simp only [CMRA.op?] at Ha'_eq
  constructor
  · exists c
    calc a' ≡{n}≡ b' • (bf • c) := Ha'_eq
         _ ≡{n}≡ (b' • bf) • c := assoc.dist
  · exact Hv'
...
instance instURFunctorAuthURF {T : COFE.OFunctorPre} [URFunctor T] : URFunctor (AuthURF (F := F) T) where
...
instance instRFunctorAuthRF {T : COFE.OFunctorPre} [URFunctor T] : RFunctor (AuthRF (F := F) T) where
  map {A A'} {B B'} _ _ _ _ f g :=
    View.mapC
      (URFunctor.map (F := T) f g).toHom
      (URFunctor.map (F := T) f g)
